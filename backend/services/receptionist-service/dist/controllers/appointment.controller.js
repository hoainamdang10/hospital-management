...
class AppointmentController {
    constructor() {
        this.getTodayAppointments = async (req, res) => {
            try {
        ...
        `)
                ...
                logger_1.default.error('Error in getTodayAppointments:', error);
                res.status(500).json({
                    success: false,
        ...
        this.updateAppointmentNotes = async (req, res) => {
            try {
                if (!req.user || !['receptionist', 'admin'].includes(req.user.role)) {
                    res.status(403).json({
                        success: false,
                ...
                logger_1.default.error('Error in updateAppointmentNotes:', error);
                res.status(500).json({
                    success: false,
        ...
        this.rescheduleAppointment = async (req, res) => {
            try {
                if (!req.user || !['receptionist', 'admin'].includes(req.user.role)) {
                    res.status(403).json({
                        success: false,
                ...
                const { data: existingAppointment, error: checkError } = await database_config_1.supabaseAdmin
                    .from('appointments')
                    .select('doctor_id')
                ...
                if (checkError && checkError.code !== 'PGRST116') {
                    logger_1.default.error('Error checking appointment availability:', checkError);
                    res.status(500).json({
                        success: false,
                ...
                const { error: updateError } = await database_config_1.supabaseAdmin
                    .from('appointments')
                    .update({
                    ...
                    .eq('appointment_id', appointment_id);
                if (updateError) {
                    logger_1.default.error('Error rescheduling appointment:', updateError);
                    res.status(500).json({
                        success: false,
                ...
                logger_1.default.error('Error in rescheduleAppointment:', error);
                res.status(500).json({
                    success: false,
