...
expect.extend({
   ...
   */
  toBeValidVietnamesePhoneNumber(received: string) {
   ...
   */
  toBeValidBHYTNumber(received: string) {
   ...
   */
  toBeValidBHTNNumber(received: string) {
   ...
   */
  toBeValidInvoiceId(received: string) {
   ...
   */
  toBeValidDoctorId(received: string) {
   ...
   */
  toBeValidVNDAmount(received: number) {
    ...
    if (pass) {
      ...
      const issues = [];
      if (!isNumber) issues.push('must be a number');
      if (!isPositive) issues.push('must be positive');
      if (!isInteger) issues.push('must be an integer');
      if (!isReasonable) issues.push('must be <= 1,000,000,000 VND');
   ...
   */
  toContainVietnameseText(received: string) {
   ...
   */
  toHaveRequiredBillingFields(received: any) {
   ...
   */
  toHaveValidInsuranceStructure(received: any) {
    if (!received) {
      return {
    ...
    if (pass) {
      ...
      if (!hasValidType) issues.push(`invalid type: ${received.type}`);
      if (!hasValidCoverage) issues.push(`invalid coverage level: ${received.coverageLevel}`);
   ...
   */
  toHaveValidPaymentStructure(received: any) {
    if (!received) {
      return {
    ...
    if (pass) {
      ...
      const issues = [];
      if (missingFields.length > 0) issues.push(`missing fields: ${missingFields.join(', ')}`);
      if (!hasValidMethod) issues.push(`invalid payment method: ${received.paymentMethod}`);
      if (!hasValidCurrency) issues.push(`invalid currency: ${received.currency}`);
      if (!hasValidAmount) issues.push(`invalid amount: ${received.amount}`);
   ...
   */
  toContainValidServiceCategories(received: string[]) {
   ...
   */
  toBeWithinVietnameseBusinessHours(received: Date) {
   ...
   */
  toHaveVietnameseErrorMessage(received: any) {
