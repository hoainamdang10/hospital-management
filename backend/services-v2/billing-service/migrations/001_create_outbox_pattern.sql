-- =====================================================
-- Migration: 001_create_outbox_pattern.sql
-- Description: Create Outbox Pattern tables for Billing Service
-- Author: Hospital Management Team
-- Date: 2025-11-03
-- Compliance: Transactional Outbox Pattern, Payment Security
-- =====================================================
...
CREATE TABLE IF NOT EXISTS outbox (
  ...
  aggregate_type VARCHAR(100) NOT NULL, -- 'BillingAggregate', 'Payment', 'InsuranceClaim', etc.
  aggregate_id VARCHAR(255) NOT NULL, -- Invoice ID format: INV-YYYYMM-XXXXXX
...
-- Index for event type in DLQ
CREATE INDEX idx_dlq_event_type
ON outbox_dead_letter_queue (event_type, moved_to_dlq_at);
...
BEGIN
  ...
  IF FOUND THEN
    ...
    -- Delete from outbox
    DELETE FROM outbox WHERE event_id = p_event_id;
  END IF;
END;
$$ LANGUAGE plpgsql;
...
-- =====================================================
-- FUNCTION: get_outbox_health
-- Purpose: Get health metrics for monitoring
-- =====================================================
...
AS $$
  ...
  FROM payment_schema.outbox_events
  GROUP BY status
  ORDER BY status;
$$;
...
GRANT EXECUTE ON FUNCTION get_outbox_health() TO service_role;

-- =====================================================
-- COMMENTS for Documentation
-- =====================================================

COMMENT ON TABLE outbox_events IS 'Transactional outbox pattern - stores billing/payment domain events for guaranteed delivery';
COMMENT ON TABLE outbox_processing_lock IS 'Distributed lock for coordinating multiple worker instances';
COMMENT ON TABLE outbox_dead_letter_queue IS 'Dead letter queue for permanently failed payment events requiring manual intervention';

COMMENT ON COLUMN outbox_events.event_id IS 'Unique identifier for the domain event (idempotency key)';
COMMENT ON COLUMN outbox_events.aggregate_id IS 'ID of the aggregate that produced this event (Invoice ID, Payment ID, etc.)';
...
DO $$
BEGIN
  RAISE NOTICE '✅ Migration 001: Billing Outbox Pattern created successfully';
  RAISE NOTICE '   - outbox_events';
  RAISE NOTICE '   - outbox_processing_lock';
  RAISE NOTICE '   - outbox_dead_letter_queue';
  RAISE NOTICE '   - 6 support functions';
