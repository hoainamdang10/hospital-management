...
export class SupabaseInvoiceRepository implements IInvoiceRepository {
  ...
  async save(invoice: Invoice): Promise<void> {
    ...
    if (payments.length > 0) {
      ...
      if (newPayments.length > 0) {
        ...
        if (insertError) {
          throw new Error(
            `Failed to insert new payments: ${insertError.message}`,
          );
        }
      }
      ...
      if (updatedPayments.length > 0) {
        ...
        for (const payment of updatedPayments) {
          ...
          if (updateError) {
            throw new Error(
              `Failed to update payment ${payment.payment_id}: ${updateError.message}`,
            );
          }
        }
      }
  ...
  async findAllByAppointmentId(appointmentId: string): Promise<Invoice[]> {
    ...
    return invoicesData.map((invoiceData) => {
      ...
      const invoicePayments =
        paymentsData?.filter(
          (payment) => payment.invoice_id === invoiceData.id,
        ) || [];
  ...
  async findExpiredPendingInvoices(referenceDate: Date): Promise<Invoice[]> {
