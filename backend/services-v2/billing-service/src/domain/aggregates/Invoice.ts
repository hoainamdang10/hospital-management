...
export class Invoice extends HealthcareAggregateRoot<InvoiceProps> {
  ...
  public processRefund(
    ...
    const refundAmount =
      this.props.totalAmount.amount * (refundPercentage / 100);
    ...
    const refundPayment = Payment.createRefund(
      Money.createSigned(refundAmount, this.props.totalAmount.currency),
  ...
  public completeRefund(
    ...
    if (refundAmount >= this.props.totalAmount.amount) {
      ...
      this.props.outstandingAmount = Money.zero(
        this.props.totalAmount.currency,
      );
