...
export class Invoice extends HealthcareAggregateRoot<InvoiceProps> {
  ...
  public processRefund(
    ...
    const refundPayment = Payment.createRefund(
      Money.create(refundAmount, this.props.totalAmount.currency),
      originalPaymentMethod,
      `REFUND-${this.props.id.value}-${Date.now()}`, // Transaction ID
      reason,
      refundedBy
    );
    ...
    this.props.outstandingAmount = Money.create(
   ...
   */
  public completeRefund(refundPaymentId: string, gatewayRefundId?: string): void {
    // Find the refund payment
    const refundPayment = this.props.payments.find(
      p => p.id === refundPaymentId && p.method === 'refund'
    );
    ...
    if (refundAmount >= this.props.totalAmount.amount) {
      ...
      this.props.outstandingAmount = Money.zero(this.props.totalAmount.currency);
    ...
    const originalPayment = this.props.payments.find(
      p => p.method !== 'refund' && p.amount.amount > 0
    );
