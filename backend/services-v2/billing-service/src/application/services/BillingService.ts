...
export class BillingService {
  ...
  async generateAppointmentInvoice(
    ...
    try {
      ...
      const insuranceProvider =
        request.insuranceInfo?.provider || request.insuranceInfo?.providerName;
      ...
      const insuranceData =
        insuranceProvider && isInsuranceActive
          ? {
              provider: insuranceProvider,
              policyNumber: request.insuranceInfo?.policyNumber || "",
              coveragePercentage:
                request.insuranceInfo?.coveragePercentage ||
  ...
  private getCoveragePercentByType(coverageType: string): number {
  ...
  private calculateInsuranceCoverage(
    ...
    if (!insuranceInfo) {
      this.loggerInstance.debug(
        "calculateInsuranceCoverage: No insurance info",
        { amount, category },
      );
      return 0;
    }
    ...
    if (insuranceInfo.coverageType === "self_pay") {
      this.loggerInstance.debug(
        "calculateInsuranceCoverage: Self-pay, no coverage",
        { amount, category },
      );
      return 0;
    }
    ...
    if (insuranceInfo.coverageType) {
      ...
      if (coveragePercentage > 0) {
        ...
        this.loggerInstance.debug(
          "calculateInsuranceCoverage: Using coverageType default",
          {
            amount,
            category,
    ...
    if (insuranceInfo.coverage) {
      ...
      if (coveragePercentage > 0) {
        const coverageAmount = Math.round(amount * (coveragePercentage / 100));
        this.loggerInstance.debug(
          "calculateInsuranceCoverage: Using legacy coverage object",
          {
            amount,
            category,
            coveragePercentage,
    ...
    this.loggerInstance.debug(
      "calculateInsuranceCoverage: No valid coverage found",
      {
        amount,
        category,
