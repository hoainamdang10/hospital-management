...
export class BillingService {
   ...
   */
  async generateAppointmentInvoice(request: AppointmentInvoiceRequest): Promise<Invoice> {
   ...
   */
  async generatePrescriptionInvoice(request: PrescriptionInvoiceRequest): Promise<Invoice> {
   ...
   */
  async generateLabTestInvoice(request: LabTestInvoiceRequest): Promise<Invoice> {
   ...
   */
  async generateTreatmentPlanInvoice(request: TreatmentPlanInvoiceRequest): Promise<Invoice> {
   ...
   */
  async generateMedicalRecordInvoice(request: MedicalRecordInvoiceRequest): Promise<Invoice> {
   ...
   * Helper methods
   */
  private calculateConsultationFee(serviceType: string, duration: number): number {
    const baseRates = {
    ...
    const baseRate = baseRates[serviceType as keyof typeof baseRates] || baseRates.consultation;
  ...
  private getServiceTypeDescription(serviceType: string): string {
    ...
    return descriptions[serviceType as keyof typeof descriptions] || 'Medical Service';
  }
  ...
  private calculateInsuranceCoverage(amount: number, insuranceInfo: any, category: string): number {
