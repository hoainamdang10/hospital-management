...
export class DownloadInvoiceUseCase extends BaseHealthcareUseCase<DownloadInvoiceRequest, DownloadInvoiceResponse> {
  constructor(
  ...
  protected async executeImpl(request: DownloadInvoiceRequest): Promise<DownloadInvoiceResponse> {
    try {
  ...
  private async generatePDF(billing: any, language: string): Promise<Buffer> {
  ...
  private generateHTML(billing: any, language: string): Buffer {
