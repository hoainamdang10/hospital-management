...
class SupabaseOutboxRepository {
    constructor(supabase, logger) {
        ...
        this.SCHEMA = 'billing_schema';
        // =====================================================
        // PRIVATE HELPERS
        // =====================================================
        this.mapToOutboxEvent = (row) => {
            return {
                id: row.id,
                ...
                status: row.processed_at ? IOutboxRepository_1.OutboxEventStatus.PUBLISHED : IOutboxRepository_1.OutboxEventStatus.PENDING,
     ...
     */
    async resolveDeadLetterEvent(eventId, resolvedBy, resolutionNotes) {
        try {
            const { error } = await this.supabase
                .schema(this.SCHEMA)
                .from(this.DLQ_TABLE)
     ...
     */
    async getRetryableEvents(batchSize) {
        try {
            const { data, error } = await this.supabase
                .schema(this.SCHEMA)
     ...
     */
    async cleanupPublishedEvents(retentionDays) {
        try {
            const { data, error } = await this.supabase
                .schema(this.SCHEMA)
