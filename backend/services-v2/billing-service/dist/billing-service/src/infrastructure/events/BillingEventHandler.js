...
class BillingEventHandler extends BaseEventHandler_1.BaseEventHandler {
    constructor(generateInvoiceUseCase, processPaymentUseCase, validateInsuranceUseCase, logger) {
        ...
        try {
            ...
            switch (event.eventType) {
                ...
                case 'medical-record.created':
                    return await this.handleMedicalRecordCreated(event);
    ...
    async handleAppointmentCompleted(event) {
        ...
        try {
            ...
            if (appointmentData.prescriptions && appointmentData.prescriptions.length > 0) {
                const prescriptionServices = appointmentData.prescriptions.map((prescription, index) => ({
            ...
            if (invoiceResult.success) {
                ...
                const invoiceEvent = VietnameseHealthcareEvents_1.VietnameseHealthcareEventFactory.createInvoiceGeneratedEvent({
                ...
                }, 'billing-service', { correlationId: event.metadata?.correlationId });
    ...
    async handleTestResultsReady(event) {
        ...
        try {
            ...
            if (invoiceResult.success) {
                ...
                // Publish invoice generated event
                const testInvoiceEvent = VietnameseHealthcareEvents_1.VietnameseHealthcareEventFactory.createInvoiceGeneratedEvent({
                    invoiceId: invoiceResult.invoiceId,
                    invoiceNumber: invoiceResult.invoiceNumber,
                    patientId: testData.patientId,
     ...
     */
    async handleInsuranceVerificationCompleted(event) {
        const startTime = Date.now();
        try {
     ...
     */
    async handleExternalPaymentCompleted(event) {
        const startTime = Date.now();
        try {
