...
class Payment extends entity_1.Entity {
    ...
    static create(amount, method, transactionId, id, vnpayData, status, paidAt, refundedAt, refundReason, refundedBy, gatewayRefundId) {
        const computedStatus = status || (method === "refund" ? "refund_pending" : "pending");
        return new Payment({
            id: id || "",
            amount,
            method,
     ...
     */
    static createRefund(amount, originalPaymentMethod, transactionId, reason, refundedBy, id, status, refundedAt, gatewayRefundId) {
