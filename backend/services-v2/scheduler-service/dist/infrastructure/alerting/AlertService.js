...
class AlertService {
     ...
     */
    async sendUnroutableMessageAlert(messageId, routingKey, exchange, metadata) {
        await this.send({
     ...
     */
    async sendHighUnroutableCountAlert(count, threshold, timeWindow) {
        await this.send({
     ...
     */
    async sendDeadLetterSaveFailureAlert(error, messageId) {
        await this.send({
            severity: AlertSeverity.ERROR,
