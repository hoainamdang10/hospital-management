...
class PatientMapper {
    ...
    static toDomain(patientRecord, insuranceRecord, emergencyContactRecords, consentRecords, linkRecords) {
        try {
            ...
            // Map PatientLinks
            const links = (linkRecords || []).map(record => PatientLink_1.PatientLink.reconstitute({
                otherPatientId: PatientId_1.PatientId.fromString(record.other_patient_id),
            ...
            const patientProps = {
                ...
                emergencyContacts,
                consents,
                status: PatientStatus_1.PatientStatus.fromString(patientRecord.status),
    ...
    static toPersistence(patient) {
        ...
        const patientRecord = {
            ...
            merged_into: props.mergedInto?.getValue() || null,
        ...
        const linkRecords = props.links.map(link => ({
            patient_id: props.id.getValue(),
            other_patient_id: link.otherPatientId.getValue(),
