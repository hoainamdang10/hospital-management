...
const CircuitBreaker_1 = require("@shared/infrastructure/resilience/CircuitBreaker");
const PatientMatchingService_1 = require("../services/PatientMatchingService");
/**
...
class SupabasePatientRepository {
    ...
    async save(patient) {
        return await this.circuitBreaker.execute(async () => {
            ...
            const patientIdValue = patient.getPatientId().getValue();
            // Save insurance info
            if (insuranceRecord) {
    ...
    async searchPatients(searchTerm, filters, pagination) {
        return await this.circuitBreaker.execute(async () => {
            let query = this.supabaseClient
                .from('patients')
                .select('*', { count: 'exact' })
                .or(`personal_info->>fullName.ilike.%${searchTerm}%,personal_info->>nationalId.ilike.%${searchTerm}%,contact_info->>primaryPhone.ilike.%${searchTerm}%,contact_info->>email.ilike.%${searchTerm}%`);
            if (filters?.isActive !== undefined) {
                query = query.eq('status', filters.isActive ? 'active' : 'inactive');
            }
            if (pagination) {
