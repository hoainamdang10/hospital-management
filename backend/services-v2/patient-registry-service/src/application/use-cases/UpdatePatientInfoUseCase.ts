 ...
 */
export class UpdatePatientInfoUseCase extends BaseHealthcareUseCase<UpdatePatientInfoRequest, UpdatePatientInfoResponse> {
  constructor(
   ...
   */
  protected async executeImpl(request: UpdatePatientInfoRequest): Promise<UpdatePatientInfoResponse> {
    try {
   ...
   */
  private validateRequest(request: UpdatePatientInfoRequest): { isValid: boolean; errors: string[] } {
    const errors: string[] = [];
   ...
   */
  private checkUpdateAuthorization(patient: Patient, request: UpdatePatientInfoRequest): {
    authorized: boolean;
    reason?: string;
  } {
   ...
   */
  private captureOriginalData(patient: Patient): any {
    return {
   ...
   */
  private async applyUpdates(patient: Patient, updates: UpdatePatientInfoRequest['updates']): Promise<string[]> {
