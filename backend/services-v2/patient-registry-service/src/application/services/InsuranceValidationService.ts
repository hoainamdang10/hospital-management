...
import {
  IInsuranceValidationService,
  ValidationResult,
  ExpirationCheckResult,
  InsuranceMetadata,
  InsuranceValidationServiceStatus
} from './IInsuranceValidationService';

/**
 * Insurance Validation Service Implementation
 * Validates BHYT and BHTN numbers according to Vietnamese standards
 */
export class InsuranceValidationService implements IInsuranceValidationService {
  ...
  private readonly BHYT_PRIORITY_LEVELS = ['1', '2', '3', '4', '5'];
   ...
   */
  async validateBHYTNumber(bhytNumber: string): Promise<ValidationResult> {
    const errors: string[] = [];
   ...
   */
  async validateBHTNNumber(bhtnNumber: string): Promise<ValidationResult> {
    const errors: string[] = [];
    const warnings: string[] = [];
    const metadata: InsuranceMetadata = {};
  ...
  async validateInsurance(
    ...
    if (insuranceType === 'BHYT') {
      return await this.validateBHYTNumber(policyNumber);
    } else if (insuranceType === 'BHTN') {
      return await this.validateBHTNNumber(policyNumber);
    } else {
      // For private insurance, just validate dates
      return this.validateInsuranceDates(validFrom, validTo);
    }
  }
   ...
   */
  private validateInsuranceDates(validFrom: Date, validTo: Date): ValidationResult {
    const errors: string[] = [];
    const warnings: string[] = [];
