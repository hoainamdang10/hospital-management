...
export class SupabasePatientRepository implements IPatientRepository {
  ...
  constructor(
    ...
    this.supabaseClient = this.optimizedClient.getConnection() as any;
  }
  ...
  async findById(patientId: PatientId): Promise<Patient | null> {
    ...
    return await this.circuitBreaker.execute(
      async () => {
        ...
        // Fetch related data
        const [insuranceData, emergencyContactsData, consentsData, linksData] =
          await Promise.all([
        ...
        // Map to domain
        const patient = PatientMapper.toDomain(
    ...
    return await this.circuitBreaker.execute(
      async () => {
        ...
        if (patientError) {
          ...
          throw new Error(
            `Failed to find patient by user ID: ${patientError.message}`,
          );
        }
  ...
  async findByNationalId(nationalId: string): Promise<Patient | null> {
    return await this.circuitBreaker.execute(
      async () => {
        ...
        if (patientError) {
          throw new Error(
            `Failed to find patient by national ID: ${patientError.message}`,
          );
        }
    ...
    return await this.circuitBreaker.execute(
      async () => {
        ...
        if (insuranceError) {
          ...
          throw new Error(
            `Failed to find patient by BHYT number: ${insuranceError.message}`,
          );
        }

        // Find patient by patient_id
        return await this.findById(
          PatientId.fromString(insuranceData.patient_id),
        );
      },
      async () => {
  ...
  async createFromUserEvent(userData: {
    ...
    return await this.circuitBreaker.execute(async () => {
      ...
      const { PersonalInfo } = await import(
        "../../domain/value-objects/PersonalInfo"
      );
      const { ContactInfo } = await import(
        "../../domain/value-objects/ContactInfo"
      );
      const { BasicMedicalInfo } = await import(
        "../../domain/value-objects/BasicMedicalInfo"
      );
      const { buildPersonalInfoForCreate, buildContactInfoForCreate } =
        await import("../../shared/helpers/PatientDataHelper");

      // Create patient using smart defaults for minimal data
      const personalInfoData = buildPersonalInfoForCreate({
        fullName: userData.fullName,
        dateOfBirth:
          userData.dateOfBirth?.toISOString().split("T")[0] || "2000-01-01", // Smart default
      ...
      const patient = Patient.register(
        ...
        PersonalInfo.create({
          ...
          gender: personalInfoData.gender as "male" | "female" | "other",
        }),
        ContactInfo.create(contactInfoData),
        BasicMedicalInfo.create({ knownAllergies: [] }), // Empty allergies array
  ...
  private async savePatientFallback(
    ...
    if (patientResult.error) {
      throw new Error(
        `Fallback save failed (patients): ${patientResult.error.message}`,
      );
    }
    ...
    const profileRecord = {
      ...
      emergency_medical_info:
        patientRecord.basic_medical_info?.emergencyMedicalInfo || null,
    ...
    if (profileResult.error) {
      throw new Error(
        `Fallback save failed (patient_profiles): ${profileResult.error.message}`,
      );
    }
    ...
    if (insuranceRecord) {
      ...
      if (insuranceResult.error) {
        throw new Error(
          `Fallback save failed (insurance_info): ${insuranceResult.error.message}`,
        );
      }
    }
    ...
    if (emergencyContactRecords.length > 0) {
      ...
      if (contactsResult.error) {
        throw new Error(
          `Fallback save failed (emergency_contacts): ${contactsResult.error.message}`,
        );
      }
    }
    ...
    if (consentRecords.length > 0) {
      ...
      if (consentsResult.error) {
        throw new Error(
          `Fallback save failed (patient_consents): ${consentsResult.error.message}`,
        );
      }
    }
    ...
    if (linkRecords.length > 0) {
      ...
      if (linksResult.error) {
        throw new Error(
          `Fallback save failed (patient_links): ${linksResult.error.message}`,
        );
      }
    }
  }
    ...
    return await this.circuitBreaker.execute(
      async () => {
        ...
        if (findError) {
          if (findError.code === "PGRST116") {
            this.logger.warn(
              "No patient record found for identity status event",
              {
                userId,
                newStatus,
              },
          ...
          throw new Error(
            `Failed to find patient for status update: ${findError.message}`,
          );
        }

        const previousStatus = patientRow.status as PatientStatus;
        const patientId = patientRow.patient_id;

        if (previousStatus === newStatus) {
          this.logger.debug("Patient status already up to date", {
            userId,
            patientId,
            status: previousStatus,
          });
          return { updated: false, patientId, previousStatus };
        }
        ...
        if (options?.updatedBy) {
          updatePayload.updated_by = options.updatedBy;
        }
        ...
        if (this.patientCache) {
          try {
            ...
            this.logger.warn(
              "Failed to invalidate patient cache after status update",
              {
                patientId,
                error:
                  cacheError instanceof Error
        ...
        this.logger.info("Patient status updated via identity event", {
          userId,
          patientId,
          previousStatus,
          newStatus,
        ...
        return { updated: true, patientId, previousStatus };
      },
      async () => {
        this.logger.warn("Circuit breaker fallback for updateStatusByUserId", {
          userId,
          newStatus,
        });
        return { updated: false };
      ...
      async () => {
        try {
          ...
          if (error) {
            ...
            throw new Error(
              `Failed to find patients with filters: ${error.message}`,
            );
          }
          ...
          const [insuranceMap, contactsMap, consentsMap, linksMap] =
            await Promise.all([
  ...
  > {
    return await this.circuitBreaker.execute(
      async () => {
        ...
        if (error) {
          throw new Error(
            `Failed to fetch candidate patients: ${error.message}`,
          );
        }
   ...
   */
  private async fetchEmergencyContacts(
    patientId: string,
  ...
  private async fetchInsuranceBatch(
    patientIds: string[],
  ): Promise<Map<string, InsuranceRecord | null>> {
  ...
  private async fetchEmergencyContactsBatch(
    patientIds: string[],
  ): Promise<Map<string, EmergencyContactRecord[]>> {
    if (patientIds.length === 0) {
      return new Map();
    }
  ...
  private async fetchConsentsBatch(
    patientIds: string[],
  ): Promise<Map<string, PatientConsentRecord[]>> {
    if (patientIds.length === 0) {
      return new Map();
    }
  ...
  private async getActiveInsurancePatientIds(): Promise<string[]> {
    ...
    const ids = (data || []).map(
      (record: { patient_id: string }) => record.patient_id,
    );
   ...
   * @private - Reserved for future use
   */
  private async _saveEmergencyContacts(
    patientId: string,
    contacts: Partial<EmergencyContactRecord>[],
  ): Promise<void> {
   ...
   * @private - Reserved for future use
   */
  private async _saveConsents(
    patientId: string,
    consents: Partial<PatientConsentRecord>[],
  ): Promise<void> {
   ...
   * @private - Reserved for future use
   */
  private async _saveLinks(
    patientId: string,
    links: Partial<PatientLinkRecord>[],
  ): Promise<void> {
  ...
  async getStatistics(): Promise<{
    ...
    return this.circuitBreaker.execute(async () => {
      ...
      if (insuranceError) {
        throw new Error(
          `Failed to get insurance data: ${insuranceError.message}`,
        );
      }
  ...
  async getPatientHistory(
    ...
    return this.circuitBreaker.execute(async () => {
      ...
      if (auditError) {
        ...
        throw new Error(
          `Failed to retrieve patient history: ${auditError.message}`,
        );
      }
