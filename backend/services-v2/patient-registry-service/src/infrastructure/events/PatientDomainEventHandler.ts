 ...
 */
export class PatientDomainEventHandler implements IEventHandler<DomainEvent> {
    ...
    try {
      ...
      switch (event.eventType) {
        case 'PatientRegistered':
          await this.handlePatientRegistered(event as PatientRegisteredEvent);
          break;
        ...
        case 'PatientConsentGranted':
          await this.handlePatientConsentGranted(event as PatientConsentGrantedEvent);
          break;
    ...
    try {
      ...
      if (this.hasCriticalInfoChanges(event.updatedFields)) {
        const criticalUpdateEvent = {
          ...
          eventData: {
            patientId: event.patientId,
            criticalFields: event.updatedFields.filter(field => 
              ['personalInfo', 'contactInfo', 'emergencyContacts'].includes(field)
            ),
            updatedBy: event.updatedBy
          },
          metadata: {
   ...
   */
  private async handlePatientConsentGranted(event: PatientConsentGrantedEvent): Promise<void> {
    try {
   ...
   */
  private hasCriticalInfoChanges(updatedFields: string[]): boolean {
