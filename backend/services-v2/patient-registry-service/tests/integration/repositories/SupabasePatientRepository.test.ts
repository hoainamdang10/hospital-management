...
describe("SupabasePatientRepository Integration Tests", () => {
  ...
  beforeAll(() => {
    ...
    optimizedSupabase = createOptimizedSupabaseClient({
      supabaseUrl,
      supabaseServiceKey: supabaseKey,
      serviceName: "patient-registry-service-test",
  ...
  describe("save", () => {
    it("should save patient to database", async () => {
      ...
      expect(retrieved?.getPersonalInfo().fullName).toBe(
        "Nguyễn Văn Test Integration",
      );
    });
    ...
    it("should save patient with all fields", async () => {
      ...
      expect(retrieved?.getContactInfo().email).toBe(
  ...
  describe("findByUserId", () => {
    it("should find patient by user ID", async () => {
      ...
      const testAdminId = uuidv4(); // Generate valid UUID for admin
  ...
  describe("save (update)", () => {
    it("should update patient information", async () => {
      ...
      const adminUserId = uuidv4(); // Generate valid UUID for admin
      testPatient.updatePersonalInfo(newPersonalInfo, adminUserId);
      await repository.save(testPatient);
    ...
    it("should update contact information", async () => {
      ...
      const adminUserId = uuidv4(); // Generate valid UUID for admin
      testPatient.updateContactInfo(newContactInfo, adminUserId);
      await repository.save(testPatient);
  ...
  describe("HIPAA compliance", () => {
    it("should store PHI securely", async () => {
      ...
      expect(found?.getPersonalInfo().nationalId).toBe("001234567890");
      expect(found?.getContactInfo().email).toBe(
        "integration.test@example.com",
      );
    });
