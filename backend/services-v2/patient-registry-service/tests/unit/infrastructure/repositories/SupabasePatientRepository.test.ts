...
describe("SupabasePatientRepository", () => {
  ...
  let mockOptimizedSupabaseClient: any;
  ...
  beforeEach(() => {
    ...
    // Mock OptimizedSupabaseClient
    mockOptimizedSupabaseClient = {
      from: jest.fn(),
      rpc: jest.fn(),
      close: jest.fn(),
      getConnection: jest.fn(() => mockSupabaseClient),
    } as any;
    ...
    (CircuitBreakerFactory.getBreaker as jest.Mock).mockReturnValue(
      mockCircuitBreaker,
    );
    ...
    repository = new SupabasePatientRepository(
      ...
      mockMatchingService,
      mockEventPublisher,
      undefined // patientCache is optional
    );
  });
  ...
  describe("constructor", () => {
    it("should initialize with correct Supabase configuration", () => {
      ...
      expect(CircuitBreakerFactory.getBreaker).toHaveBeenCalledWith(
        "patient-repository",
      );
    });
    ...
    it("should work without event publisher (optional dependency)", () => {
      ...
      const newRepository = new SupabasePatientRepository(
        ...
        mockLogger,
        mockMatchingService,
        undefined, // eventPublisher is optional
        undefined // patientCache is optional
      );
  ...
  describe("findById", () => {
    ...
    it("should find patient by ID successfully", async () => {
      ...
      const mockQuery = {
        ...
        single: jest
          .fn()
          .mockResolvedValue({ data: mockPatientData, error: null }),
      };
      ...
      (repository as any).fetchEmergencyContacts = jest
        .fn()
        .mockResolvedValue([]);
  ...
  describe("save", () => {
    it("should save patient successfully", async () => {
      ...
      (PatientMapper.toPersistence as jest.Mock).mockReturnValue(
        mockPersistenceData,
      );
    ...
    it("should throw error when patient ID is missing", async () => {
      ...
      // Act & Assert
      await expect(repository.save(patientWithoutId)).rejects.toThrow(
    ...
    it("should handle database errors during save", async () => {
      ...
      await expect(repository.save(testPatient)).rejects.toThrow(
        "Failed to save patient: Database constraint violation",
      );
    });
    ...
    it("should publish domain events after successful save", async () => {
      ...
      testPatient.getUncommittedEvents = jest
        .fn()
        .mockReturnValue(mockDomainEvents);
  ...
  describe("delete", () => {
    ...
    it("should handle errors during delete", async () => {
      ...
      // Act & Assert
      await expect(repository.delete(testPatientId)).rejects.toThrow(
        "Failed to delete patient: Patient not found",
      );
    });
  });
  ...
  describe("searchPatients", () => {
    ...
    it("should search patients successfully", async () => {
      ...
      // Act
      const result = await repository.searchPatients(
        searchTerm,
        filters,
        pagination,
      );
    ...
    it("should handle search errors", async () => {
      // Arrange - Mock query chain that returns error
      const mockQuery = {
        select: jest.fn().mockReturnThis(),
        or: jest.fn().mockReturnThis(),
        eq: jest.fn().mockReturnThis(),
      ...
      // Mock the final query execution to return error
      ...
      expect(mockLogger.warn).toHaveBeenCalledWith(
        "Circuit breaker fallback for searchPatients"
      );
    });
  });
  ...
  describe("getStatistics", () => {
    it("should return patient statistics successfully", async () => {
      ...
      mockSupabaseClient.from.mockImplementation((table: string) => {
            ...
            select: jest.fn((columns: string, options?: any) => {
              ...
              if (columns === "personal_info") {
                return Promise.resolve(
                  personalInfoResponses.shift() ?? { data: [], error: null },
                );
              }
              if (columns === "status") {
              ...
              if (columns === "created_at") {
                return {
                  gte: jest.fn(() =>
                    Promise.resolve({ data: trendData, error: null }),
                  ),
                };
              }
        ...
        if (table === "insurance_info") {
          return {
            select: jest.fn(() =>
              Promise.resolve({ data: insuranceData, error: null }),
            ),
          };
        }
    ...
    it("should handle statistics errors", async () => {
      ...
      await expect(repository.getStatistics()).rejects.toThrow(
        "Failed to get total count: Total failed",
      );
    });
  });
