...
> {
  ...
  protected async executeInternal(
    ...
    try {
      ...
      const newStartTimeUtc = convertClinicLocalToUtc(
        request.newAppointmentDate,
        request.newAppointmentTime,
      );
      const newEndTimeUtc = new Date(
        newStartTimeUtc.getTime() + appointment.getDurationMinutes() * 60000,
      );
      const newTimeSlot = TimeSlot.createWithTimestamps(
        request.newAppointmentDate,
        request.newAppointmentTime,
        newStartTimeUtc,
        newEndTimeUtc,
      ...
      const canReschedule =
        await this.authorizationService.canRescheduleAppointment(
      ...
      try {
        ...
        console.log(
          `[RescheduleAppointment] Reminders rescheduled for appointment ${request.appointmentId}`,
        );
      } catch (reminderError) {
        // Log but don't fail the reschedule
        console.error(
          "[RescheduleAppointment] Failed to reschedule reminders:",
          reminderError,
        );
      }
  ...
  private validateReschedule(
    ...
    const hoursUntilNewAppointment =
      (newDateTime.getTime() - now.getTime()) / (1000 * 60 * 60);
  ...
  private async checkTimeSlotAvailability(
    ...
    try {
      ...
      const startOfDay = convertClinicLocalToUtc(
        timeSlot.appointmentDate,
        "00:00:00",
      );
      const endOfDay = convertClinicLocalToUtc(
        timeSlot.appointmentDate,
        "23:59:59",
      );

      const existingAppointments =
        await this.appointmentRepository.findByTimeSlot(
          doctorId,
          startOfDay,
          endOfDay,
        );
      ...
      for (const existing of existingAppointments) {
        ...
        const existingEndTime = existing.timeSlot.getEndTime(
          existing.durationMinutes,
        );

        // Check for overlap
        if (
          (newAppointmentTime >= existingTime &&
