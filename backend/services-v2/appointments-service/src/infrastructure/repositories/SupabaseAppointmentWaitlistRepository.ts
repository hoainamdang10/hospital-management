...
import { 
  ...
  WaitlistFilterCriteria 
} from '../../domain/repositories/IAppointmentWaitlistRepository';
 ...
 */
export class SupabaseAppointmentWaitlistRepository implements IAppointmentWaitlistRepository {
  private readonly tableName = 'appointment_waitlist';
   ...
   */
  private toDomain(row: WaitlistRow): AppointmentWaitlist {
    const props: AppointmentWaitlistProps = {
      ...
      priority: row.priority as WaitlistPriority,
    ...
    return AppointmentWaitlist.reconstitute(props);
  }

  /**
   * Map domain entity to database row
   */
  private toDatabase(waitlist: AppointmentWaitlist): Partial<WaitlistRow> {
    return {
   ...
   */
  async save(waitlist: AppointmentWaitlist): Promise<void> {
   ...
   */
  async update(waitlist: AppointmentWaitlist): Promise<void> {
    const row = this.toDatabase(waitlist);
   ...
   */
  async findById(waitlistId: string): Promise<AppointmentWaitlist | null> {
    ...
    return data ? this.toDomain(data as WaitlistRow) : null;
  }

  /**
   * Find waitlist entries by patient ID
   */
  async findByPatientId(patientId: string): Promise<AppointmentWaitlist[]> {
    const { data, error } = await this.supabase
      .from(`${this.schema}.${this.tableName}`)
      .select('*')
    ...
    return (data as WaitlistRow[]).map(row => this.toDomain(row));
  }
  ...
  async findWithFilters(
    criteria: WaitlistFilterCriteria,
    limit: number = 50,
    offset: number = 0
   ...
   */
  async findWaitingEntries(limit: number = 100): Promise<AppointmentWaitlist[]> {
    const { data, error } = await this.supabase
      .from(`${this.schema}.${this.tableName}`)
      .select('*')
   ...
   */
  async findExpiredEntries(limit: number = 100): Promise<AppointmentWaitlist[]> {
    const { data, error } = await this.supabase
      .from(`${this.schema}.${this.tableName}`)
      .select('*')
      .eq('status', WaitlistStatus.WAITING)
   ...
   */
  async delete(waitlistId: string): Promise<void> {
    const { error } = await this.supabase
      .from(`${this.schema}.${this.tableName}`)
      .delete()
      .eq('waitlist_id', waitlistId);
   ...
   */
  async count(criteria: WaitlistFilterCriteria): Promise<number> {
    let query = this.supabase
      .from(`${this.schema}.${this.tableName}`)
  ...
  async getWaitlistPosition(waitlistId: string): Promise<number> {
    ...
    const { count, error } = await this.supabase
      ...
      .or(`expires_at.is.null,expires_at.gt.${new Date().toISOString()}`)
      .or(
        `priority.gt.${entry.priority},and(priority.eq.${entry.priority},created_at.lt.${entry.createdAt.toISOString()})`
      );
