...
export class ConflictResolutionService implements IConflictResolutionService {
    ...
    try {
      ...
      while (currentDate < endTime) {
        ...
        const hasConflict = dayAppointments.some(apt => {
          ...
          return (
            (currentDate < aptEnd && slotEnd > aptStart) // Overlap check
          );
        });
  ...
  async findUrgentAppointmentSlot(criteria: {
    ...
    try {
      ...
      const availableProviders = await this.findAvailableProvidersForUrgent(
   ...
   */
  private async findAvailableProvidersForUrgent(
    ...
    try {
      ...
      for (let i = 0; i < 8; i++) {
        ...
        const isAvailable = await this.appointmentRepository.checkStaffAvailability(
          'mock-provider-id', // In production, this would be actual provider IDs
          slotStart,
          slotEnd
        );
