...
interface InvoiceExpiredEventData {
  invoiceId?: string;
  appointmentId?: string;
  patientId?: string;
  reason?: string;
...
export class BillingEventConsumer {
    ...
    try {
      ...
      switch (eventType) {
        ...
        case "invoice.expired":
          await this.handleInvoiceExpired(
            this.buildInvoiceExpiredPayload(eventData),
          );
          break;
  ...
  private buildInvoiceExpiredPayload(event: any): InvoiceExpiredEventData {
    const expiredAt = event?.expiredAt || event?.expired_at;
    return {
      invoiceId: event?.invoiceId || event?.invoice_id,
      appointmentId:
        event?.appointmentId || event?.appointment_id || event?.aggregateId,
  ...
  private async handleInvoiceExpired(
    data: InvoiceExpiredEventData,
  ): Promise<void> {
    if (!data.appointmentId) {
      console.warn(
        "Received billing.invoice.expired event without appointmentId",
        data,
      );
      return;
    }
    ...
    try {
      ...
      if (!appointment) {
        console.warn(
          `InvoiceExpired event ignored - appointment ${data.appointmentId} not found`,
        );
        return;
      }
      ...
      if (
        ...
        console.info(
          `InvoiceExpired event ignored - appointment ${data.appointmentId} already ${appointment.status}`,
        );
        return;
      }

      appointment.cancel(
        data.reason || "Invoice expired - payment timeout",
        "system",
      );
      await this.appointmentRepository.save(appointment);
      console.info(
        `Appointment ${data.appointmentId} cancelled due to billing invoice expiration`,
      );
    } catch (error) {
      console.error(
        `Failed to process billing.invoice.expired for appointment ${data.appointmentId}`,
        error,
      );
      throw error;
    }
