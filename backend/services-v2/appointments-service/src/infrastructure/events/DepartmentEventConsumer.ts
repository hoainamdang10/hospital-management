 ...
 */
export class DepartmentEventConsumer {
  private connection?: any;
  private channel?: any;
  private isConnected = false;

  constructor(
    private config: DepartmentEventConsumerConfig,
    ...
    try {
      ...
      switch (routingKey) {
        ...
        case 'department.resource.updated':
          await this.handleDepartmentResourceUpdated(event.payload as DepartmentResourceUpdatedEventData);
          break;

        case 'department.operational_hours.changed':
          await this.handleDepartmentOperationalHoursChanged(event.payload as DepartmentOperationalHoursChangedEventData);
          break;

        case 'department.capacity.updated':
          await this.handleDepartmentCapacityUpdated(event.payload as DepartmentCapacityUpdatedEventData);
          break;
   ...
   */
  private async handleDepartmentResourceUpdated(data: DepartmentResourceUpdatedEventData): Promise<void> {
   ...
   */
  private async handleDepartmentOperationalHoursChanged(data: DepartmentOperationalHoursChangedEventData): Promise<void> {
    ...
    try {
      ...
      const conflictingAppointments = await this.findAppointmentsConflictingWithHours(
        data.departmentId,
        data.newHours
      );
   ...
   */
  private async handleDepartmentCapacityUpdated(data: DepartmentCapacityUpdatedEventData): Promise<void> {
    ...
    try {
      ...
      const overloadedAppointments = await this.findAppointmentsExceedingCapacity(
        data.departmentId,
        data.newCapacity
      );
  ...
  private async checkDepartmentCapacityCompliance(departmentData: DepartmentCreatedEventData): Promise<void> {
    try {
      ...
      const todayAppointments = await this.appointmentRepository.findByDepartmentAndDate(
   ...
   */
  private async handleOperatingHoursConflict(appointment: any, hoursData: any): Promise<void> {
    try {
      ...
      await this.reminderService.sendRescheduleNotification(
        ...
        'Department operating hours have changed'
      );
   ...
   */
  private async findAppointmentsConflictingWithHours(
    departmentId: string,
    newHours: any
  ): Promise<any[]> {
    try {
   ...
   */
  private async updateDepartmentAppointmentAvailability(departmentId: string, newHours: any): Promise<void> {
   ...
   */
  private async notifyPatientsAboutOperatingHoursChanges(
    appointments: any[],
    hoursData: any
  ): Promise<void> {
    try {
      for (const appointment of appointments) {
        await this.reminderService.sendOperatingHoursChangeNotification(
          hoursData.departmentId,
          hoursData.newHours,
   ...
   */
  private async findAppointmentsExceedingCapacity(
    departmentId: string,
    newCapacity: any
  ): Promise<any[]> {
    try {
   ...
   */
  private async handleCapacityConflict(appointment: any, capacityData: any): Promise<void> {
    try {
      ...
      await this.reminderService.sendRescheduleNotification(
        ...
        appointment.patientId,
        new Date(appointment.startTime),
        'Department capacity limits have been updated'
      );
   ...
   */
  private async updateDepartmentCapacitySettings(departmentId: string, newCapacity: any): Promise<void> {
   ...
   */
  private async processWaitlistForIncreasedCapacity(capacityData: any): Promise<void> {
   ...
   * DEPRECATED: Waitlist management belongs to Waitlist Service
   */
  private async scheduleFromWaitlist(waitlistEntry: any, timeSlot: any): Promise<void> {
   ...
   */
  private async handleFacilityResourceUpdate(resourceData: DepartmentResourceUpdatedEventData): Promise<void> {
    try {
   ...
   * DEPRECATED: Resource management belongs to Resource Management Service
   */
  private async handleEquipmentResourceUpdate(resourceData: DepartmentResourceUpdatedEventData): Promise<void> {
    try {
   ...
   */
  private async handleBedResourceUpdate(resourceData: DepartmentResourceUpdatedEventData): Promise<void> {
    try {
   ...
   */
  private async handleResourceConflict(appointment: any, resourceData: any): Promise<void> {
    try {
      // FIXED: Use proper aggregate pattern instead of direct update
      const appointmentAggregate = await this.appointmentRepository.findByIdString(appointment.id);
      if (!appointmentAggregate) {
      ...
      await this.reminderService.sendRescheduleNotification(
        appointment.id,
        appointment.patientId,
        new Date(), // Reschedule time - will be determined later
   ...
   */
  private async handleDepartmentOverload(departmentId: string, appointments: any[]): Promise<void> {
    try {
   ...
   */
  private async updateStaffAppointmentAvailability(staffData: DepartmentStaffAssignedEventData): Promise<void> {
    try {
   ...
   */
  private async processPendingAppointmentsForStaff(staffData: DepartmentStaffAssignedEventData): Promise<void> {
    try {
      // FIXED: Remove findPendingByDepartment() - use proper appointment queries
      // Get appointments that could be assigned to this staff
      const pendingAppointments = await this.appointmentRepository.findByDepartmentId(
      ...
      const unassignedAppointments = pendingAppointments.filter(apt => 
   ...
   */
  private async isStaffSuitableForAppointment(staffData: any, appointment: any): Promise<boolean> {
    try {
      ...
      const isAvailable = await this.appointmentRepository.checkStaffAvailability(
        staffData.staffId,
        appointmentDateTime,
        endTime
      );
   ...
   * FIXED: Use proper aggregate pattern and method calls
   */
  private async assignAppointmentToStaff(appointment: any, staffData: any): Promise<void> {
    try {
      // FIXED: Use proper aggregate pattern instead of direct update
      const appointmentAggregate = await this.appointmentRepository.findByIdString(appointment.id);
      if (!appointmentAggregate) {
      ...
      // FIXED: Use proper method signature for reminder service
      await this.reminderService.sendStaffAssignmentNotification(
        appointment.id,
        staffData.staffId,
        'assigned'
      );
