...
import { IReschedulingQueueRepository, ReschedulingStatus, ReschedulingPriority, PatientResponse } from '../../domain/interfaces/IReschedulingQueueRepository';
...
export class ReschedulingQueueController {
  constructor(private dependencies: ReschedulingQueueControllerDependencies) {}
   ...
   */
  async getPendingEntries(req: Request, res: Response): Promise<void> {
    try {
      const { doctorId, priority, limit = 50, offset = 0 } = req.query;

      const query = {
        doctorId: doctorId as string,
        priority: priority as ReschedulingPriority | undefined,
        limit: Number(limit),
   ...
   */
  async updatePatientResponse(req: Request, res: Response): Promise<void> {
    try {
      const { id } = req.params;
      ...
      if (!Object.values(PatientResponse).includes(patientResponse as PatientResponse)) {
        res.status(400).json({
          success: false,
          error: 'Invalid patient response',
          validResponses: Object.values(PatientResponse)
        });
        return;
      }
   ...
   */
  async processExpiredEntries(req: Request, res: Response): Promise<void> {
    try {
   ...
   */
  async getEntriesByDoctor(req: Request, res: Response): Promise<void> {
    try {
      ...
      const query = {
        status: status as ReschedulingStatus,
        priority: priority as ReschedulingPriority,
        limit: Number(limit),
        offset: Number(offset)
      };
   ...
   */
  async completeRescheduling(req: Request, res: Response): Promise<void> {
    try {
      const { id } = req.params;
      ...
      const completedEntry = await this.dependencies.reschedulingService.completeRescheduling(
        id,
        newAppointmentId,
        resolvedBy
      );
