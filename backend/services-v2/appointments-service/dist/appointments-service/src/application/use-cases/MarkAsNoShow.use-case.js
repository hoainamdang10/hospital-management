...
exports.MarkAsNoShowUseCase = void 0;
 ...
 */
class MarkAsNoShowUseCase extends use_case_interface_1.BaseHealthcareUseCase {
    constructor(appointmentRepository, authorizationService) {
        super();
        this.appointmentRepository = appointmentRepository;
        this.authorizationService = authorizationService;
        this.MIN_MINUTES_AFTER_APPOINTMENT = 15;
        this.NO_SHOW_PENALTY_FEE = 100000; // 100,000 VND
    }
    async executeInternal(request) {
        try {
            // 1. Authorization check - only staff can mark as no-show
            const hasRole = await this.authorizationService.hasAnyRole(request.markedBy, [IAuthorizationService_1.UserRole.SUPER_ADMIN, IAuthorizationService_1.UserRole.ADMIN, IAuthorizationService_1.UserRole.DOCTOR, IAuthorizationService_1.UserRole.NURSE]);
            if (!hasRole) {
                throw new IAuthorizationService_1.AuthorizationError('You are not authorized to mark appointments as no-show', request.markedBy, 'mark_no_show', request.appointmentId);
            }
            // 2. Find appointment
...
exports.MarkAsNoShowUseCase = MarkAsNoShowUseCase;
//# sourceMappingURL=MarkAsNoShow.use-case.js.map