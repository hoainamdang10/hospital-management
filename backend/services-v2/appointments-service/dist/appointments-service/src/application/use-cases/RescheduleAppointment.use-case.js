...
class RescheduleAppointmentUseCase extends use_case_interface_1.BaseHealthcareUseCase {
    ...
    async executeInternal(request) {
        try {
            ...
            const newTimeSlot = TimeSlot_vo_1.TimeSlot.create(request.newAppointmentDate, request.newAppointmentTime);
            // 4. Check if new time slot is available
            const isAvailable = await this.checkTimeSlotAvailability(appointment.doctorId, newTimeSlot, request.appointmentId);
            if (!isAvailable) {
                return {
                    success: false,
     ...
     */
    async checkTimeSlotAvailability(doctorId, timeSlot, excludeAppointmentId) {
        try {
            ...
            const newAppointmentTime = new Date(`${timeSlot.appointmentDate}T${timeSlot.appointmentTime}`);
            for (const existing of existingAppointments) {
                ...
                // Check for overlap
                if ((newAppointmentTime >= existingTime && newAppointmentTime < existingEndTime) ||
