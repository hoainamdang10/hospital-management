...
if (!SUPABASE_URL || !SUPABASE_SERVICE_ROLE_KEY) {
    console.warn("[ChatRoutes] Missing Supabase credentials. Chat API will not work.");
}
const supabase = (0, supabase_js_1.createClient)(SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, { db: { schema: "appointments_schema" } });
...
function parseRoles(raw) {
    try {
        return raw ? JSON.parse(raw).map((r) => r.toLowerCase()) : [];
    }
    catch {
        return [];
    }
}
function isParticipant(userId, patientId, doctorId, patientHeader, providerHeader, roles) {
    ...
    if (patientHeader && patientHeader === patientId)
        return true;
    if (providerHeader && providerHeader === doctorId)
        return true;
    // Fallback: match by userId if schemas overlap (less reliable)
    if (userId && (userId === patientId || userId === doctorId))
        return true;
    return false;
}
function createChatRoutes() {
    ...
    router.get("/chat/conversations", async (req, res) => {
        try {
            ...
            const patientHeader = req.header("x-patient-id");
            const providerHeader = req.header("x-provider-id");
            const roles = parseRoles(req.header("x-user-roles"));
            if (!appointmentId) {
                res
                    .status(400)
            ...
            const { patient_id, doctor_id } = participants;
            if (!isParticipant(userId, patient_id, doctor_id, patientHeader, providerHeader, roles)) {
    ...
    router.get("/chat/conversations/:id/messages", async (req, res) => {
        try {
            ...
            if (!isParticipant(userId, convo.patient_id, convo.doctor_id, patientHeader, providerHeader, roles)) {
                res.status(403).json({ success: false, error: "Forbidden" });
                return;
            }
            const { data, error } = await supabase
    ...
    router.post("/chat/messages", async (req, res) => {
        try {
            ...
            const patientId = convo.patient_id;
            const doctorId = convo.doctor_id;
            if (!isParticipant(userId, patientId, doctorId, patientHeader, providerHeader, roles)) {
                res.status(403).json({ success: false, error: "Forbidden" });
                return;
            }
            const isPatientSender = (userId && userId === patientId) ||
                (!!patientHeader && patientHeader === patientId) ||
                roles.includes("patient");
            const isDoctorSender = (userId && userId === doctorId) ||
