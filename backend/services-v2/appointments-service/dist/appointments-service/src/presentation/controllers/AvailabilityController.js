...
class AvailabilityController {
    constructor(findAvailableTimeSlotsUseCase, providerScheduleRepository, httpProviderService) {
        this.findAvailableTimeSlotsUseCase = findAvailableTimeSlotsUseCase;
    ...
    async getProviderSchedule(req, res) {
        try {
            ...
            let schedule = await this.providerScheduleRepository.findByProviderId(providerId);
            if (!schedule) {
                console.warn(`[AvailabilityController] Cache miss for provider ${providerId}, fetching from Provider Service`);
                ...
                if (schedule) {
                    try {
                        ...
                        console.error("[AvailabilityController] Failed to upsert provider schedule cache:", cacheError);
                    }
                }
            }
            if (!schedule) {
