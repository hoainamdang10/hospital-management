...
exports.SupabaseQueueRepository = void 0;
const supabase_js_1 = require("@supabase/supabase-js");
const Queue_aggregate_1 = require("../../domain/aggregates/Queue.aggregate");
const QueueEntry_entity_1 = require("../../domain/entities/QueueEntry.entity");
class SupabaseQueueRepository {
    constructor(supabaseUrl, supabaseKey) {
        this.queuesTable = 'queues';
        this.entriesTable = 'queue_entries';
     ...
     */
    async findOrCreateByDoctorAndDate(doctorId, date) {
        try {
            // Try to find existing queue
            let queue = await this.findByDoctorAndDate(doctorId, date);
            if (!queue) {
                // Create new queue
                queue = Queue_aggregate_1.Queue.create(doctorId, date);
     ...
     */
    toDomainAggregate(queueRow, entriesRows) {
        // Map entries to QueueEntry domain entities
        const entries = entriesRows.map(row => QueueEntry_entity_1.QueueEntry.reconstitute({
            id: row.id,
        ...
        return Queue_aggregate_1.Queue.reconstitute({
...
//# sourceMappingURL=SupabaseQueueRepository.js.map