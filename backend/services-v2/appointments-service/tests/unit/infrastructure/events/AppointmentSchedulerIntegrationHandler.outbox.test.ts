...
describe("AppointmentScheduledSchedulerHandler → Outbox", () => {
  it("enqueues SchedulerReminderCreate with dedup key per reminder window", async () => {
    ...
    const handler = new AppointmentScheduledSchedulerHandler(
      outbox as any,
      "tenant-x",
    );
    ...
    const evt = makeDomainEvent({
      ...
      doctorId: "CARD-DOC-250101-001",
      appointmentDate: start.toISOString().split('T')[0],
      appointmentTime: start.toTimeString().split(' ')[0].substring(0, 8),
      durationMinutes: 30,
      type: "CONSULTATION",
      reason: "Checkup",
    ...
    const found = outbox.enqueued.find(
      (x) => x.dedupKey === "2025-APT-000001-001:reminder-24h",
    );
    expect(found).toBeTruthy();
    expect(found.payload.ownerService).toBe("appointments");
    expect(found.payload.ownerResourceId).toBe("2025-APT-000001-001");
    expect(found.payload.topicOrCommand).toContain(
