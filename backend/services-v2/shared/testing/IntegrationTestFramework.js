...
class IntegrationTestFramework {
    ...
    async setupTestAuthentication() {
        ...
        for (const user of testUsers) {
            try {
                const response = await axios_1.default.post(`${this.config.services.apiGateway}/api/v1/auth/login`, {
                    email: user.email,
    ...
    async runAuthenticationTests() {
        ...
        const protectedAccessTest = await this.executeTest({
            ...
            testFunction: async () => {
                ...
                const response = await axios_1.default.get(`${this.config.services.apiGateway}/api/v1/auth/profile`, {
    ...
    async runPatientJourneyTests() {
        ...
        const patientRegistrationTest = await this.executeTest({
            ...
            testFunction: async () => {
                ...
                const response = await axios_1.default.post(`${this.config.services.apiGateway}/api/v1/patients`, patientData, { headers: { Authorization: `Bearer ${token}` } });
                return {
                    assertions: [
                        {
     ...
     */
    async runAppointmentBillingTests() {
        ...
        const invoiceGenerationTest = await this.executeTest({
            ...
            testFunction: async () => {
                ...
                const response = await axios_1.default.post(`${this.config.services.apiGateway}/api/v1/billing/invoices`, invoiceData, { headers: { Authorization: `Bearer ${token}` } });
                return {
                    assertions: [
                        {
                        ...
                        },
                        {
                            ...
                            expected: true,
                            actual: response.data.invoice.items.some((item) => item.description.includes('Khám tổng quát')),
                            passed: response.data.invoice.items.some((item) => item.description.includes('Khám tổng quát'))
                        }
                    ]
                };
            }
    ...
    async runNotificationTests() {
        ...
        const notificationTest = await this.executeTest({
            ...
            testFunction: async () => {
                ...
                const response = await axios_1.default.post(`${this.config.services.apiGateway}/api/v1/notifications/send`, notificationData, { headers: { Authorization: `Bearer ${token}` } });
                return {
                    assertions: [
                        {
    ...
    async runWorkflowIntegrationTests() {
        ...
        const workflowTest = await this.executeTest({
            ...
            testFunction: async () => {
                const orchestrator = WorkflowOrchestrator_1.WorkflowOrchestrator.getInstance();
     ...
     */
    async runVietnameseHealthcareTests() {
     ...
     */
    async runCrossServiceCommunicationTests() {
        ...
        const apiGatewayTest = await this.executeTest({
            ...
            testFunction: async () => {
                ...
                const results = await Promise.all(serviceEndpoints.map(async (endpoint) => {
                    try {
                        const response = await axios_1.default.get(`${this.config.services.apiGateway}${endpoint}`, {
                            headers: { Authorization: `Bearer ${token}` },
                            timeout: 5000
                        });
     ...
     */
    calculateEndpointCoverage(tests) {
        const testedEndpoints = new Set(tests.map(t => t.metadata.serviceName).filter(s => s !== 'all-services'));
        return testedEndpoints.size;
    }
    /**
     * Calculate workflow coverage
     */
    calculateWorkflowCoverage(tests) {
        const workflowTests = tests.filter(t => t.metadata.testCategory === 'WORKFLOW' ||
     ...
     */
    calculateHealthcareCoverage(tests) {
        const healthcareTests = tests.filter(t => t.metadata.testCategory === 'VIETNAMESE_HEALTHCARE' ||
     ...
     * Generate test report
     */
    async generateTestReport(testSuite) {
...
//# sourceMappingURL=IntegrationTestFramework.js.map