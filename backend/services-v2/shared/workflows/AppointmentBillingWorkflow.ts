...
export class AppointmentBillingWorkflow {
  private static instance: AppointmentBillingWorkflow;
  ...
  public static getInstance(): AppointmentBillingWorkflow {
   ...
   */
  private registerAppointmentInvoiceGenerationWorkflow(): void {
    const workflow: WorkflowDefinition = {
      ...
      description:
        "Generate invoice for appointment services with Vietnamese healthcare compliance",
      vietnameseDescription:
        "Tạo hóa đơn cho dịch vụ khám bệnh tuân thủ quy định y tế Việt Nam",
      steps: [
        {
   ...
   */
  private registerInsuranceClaimProcessingWorkflow(): void {
    const workflow: WorkflowDefinition = {
      ...
      description:
        "Process insurance claims for BHYT and BHTN with Vietnamese healthcare standards",
      vietnameseDescription:
        "Xử lý yêu cầu bảo hiểm BHYT và BHTN theo tiêu chuẩn y tế Việt Nam",
      steps: [
        {
   ...
   */
  private registerPaymentProcessingWorkflow(): void {
    const workflow: WorkflowDefinition = {
      workflowId: "payment-processing",
      workflowName: "Payment Processing Workflow",
      vietnameseWorkflowName: "Quy trình Xử lý Thanh toán",
      description:
        "Process patient payments through various methods including PayOS integration",
      vietnameseDescription:
        "Xử lý thanh toán bệnh nhân qua nhiều phương thức bao gồm tích hợp PayOS",
      steps: [
        {
   ...
   */
  private registerBillingReconciliationWorkflow(): void {
    const workflow: WorkflowDefinition = {
      ...
      description:
        "Reconcile billing records with payments and insurance claims",
      vietnameseDescription:
        "Đối soát hồ sơ thanh toán với các khoản thanh toán và yêu cầu bảo hiểm",
      steps: [
        {
   ...
   */
  private registerRefundProcessingWorkflow(): void {
    const workflow: WorkflowDefinition = {
      ...
      vietnameseDescription:
        "Xử lý hoàn tiền cho các cuộc hẹn bị hủy hoặc thanh toán thừa",
      steps: [
        {
   ...
   */
  private registerBHYTIntegrationWorkflow(): void {
    const workflow: WorkflowDefinition = {
      workflowId: "bhyt-integration",
      workflowName: "BHYT Integration Workflow",
      vietnameseWorkflowName: "Quy trình Tích hợp BHYT",
      description:
        "Integration with Vietnamese Social Health Insurance (BHYT) system",
      vietnameseDescription:
        "Tích hợp với hệ thống Bảo hiểm Y tế Xã hội Việt Nam (BHYT)",
      steps: [
        {
   ...
   */
  private registerPayOSIntegrationWorkflow(): void {
    const workflow: WorkflowDefinition = {
      workflowId: "payos-integration",
      workflowName: "PayOS Integration Workflow",
      vietnameseWorkflowName: "Quy trình Tích hợp PayOS",
      description:
        "Integration with PayOS payment gateway for Vietnamese healthcare payments",
      vietnameseDescription:
        "Tích hợp với cổng thanh toán PayOS cho thanh toán y tế Việt Nam",
      steps: [
        {
   ...
   */
  public async executeAppointmentInvoiceGeneration(
    billingContext: BillingContext
  ): Promise<any> {
    console.log(
      `💰 Starting Invoice Generation for appointment: ${billingContext.appointmentId}`
    );
   ...
   */
  public async executeInsuranceClaimProcessing(
    billingContext: BillingContext
  ): Promise<any> {
    console.log(
   ...
   */
  public async executePaymentProcessing(
    billingContext: BillingContext,
   ...
   */
  public async executeCompleteAppointmentBilling(
    billingContext: BillingContext
  ): Promise<any> {
    console.log(
    ...
    try {
      // 1. Generate Invoice
      const invoiceResult =
        await this.executeAppointmentInvoiceGeneration(billingContext);
      ...
      if (billingContext.insuranceInfo) {
        const insuranceResult =
          await this.executeInsuranceClaimProcessing(billingContext);
   ...
   */
  public getAppointmentBillingStatus(): any {
    return {
      registeredWorkflows: [
        "appointment-invoice-generation",
        "insurance-claim-processing",
