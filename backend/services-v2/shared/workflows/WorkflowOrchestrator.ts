...
import { EventBusConfiguration, IntegrationEvent } from '../events/EventBusConfiguration';
...
export class WorkflowOrchestrator extends BaseEventHandler {
   ...
   * Execute workflow
   */
  private async executeWorkflow(instance: WorkflowInstance): Promise<WorkflowExecutionResult> {
    const startTime = Date.now();
   ...
   */
  private createServiceEvent(instance: WorkflowInstance, step: WorkflowStep): IntegrationEvent {
    return {
   ...
   */
  private async executeServiceCall(event: IntegrationEvent): Promise<any> {
    try {
    ...
    try {
      ...
      for (let i = instance.currentStepIndex - 1; i >= 0; i--) {
        ...
        if (originalStep.status === WorkflowStepStatus.COMPLETED && originalStep.compensationAction) {
          const compensationStep: WorkflowStep = {
   ...
   */
  private extractWorkflowOutput(instance: WorkflowInstance): any {
    const output: any = {
   ...
   */
  private updateWorkflowMetrics(workflowId: string, result: WorkflowExecutionResult): void {
   ...
   */
  protected async processEvent(event: IntegrationEvent): Promise<any> {
   ...
   * Get workflow status
   */
  public getWorkflowStatus(instanceId: string): WorkflowInstance | null {
