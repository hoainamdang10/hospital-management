...
describe('RabbitMQEventPublisher', () => {
  ...
  describe('connect', () => {
    ...
    it('should not connect twice', async () => {
      ...
      expect(mockLogger.warn).toHaveBeenCalledWith(
        expect.stringContaining('connection already established')
      );
    });
  });
  ...
  describe('publish', () => {
    ...
    it('should throw error if not connected', async () => {
      ...
      expect(mockLogger.error).toHaveBeenCalledWith(
        'Cannot publish event: RabbitMQ not connected',
        expect.objectContaining({
          eventType: event.eventType
        })
      );
    ...
    it('should throw error after max retries', async () => {
      ...
      expect(mockChannel.publish).toHaveBeenCalledTimes(options.maxRetries + 1);
    });
  ...
  describe('publishAll', () => {
    ...
    it('should continue publishing even if one fails', async () => {
      ...
      await expect(eventPublisher.publishAll(events)).resolves.toBeUndefined();
