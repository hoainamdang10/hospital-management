/**
 * DeactivateStaffUseCase Unit Tests
 * Tests for staff deactivation use case
 */
...
describe('DeactivateStaffUseCase', () => {
  ...
  const validRequest: DeactivateStaffRequest = {
    staffId: 'DOC-CARDIO-202501-001',
    reason: 'Resignation',
    deactivatedBy: 'admin-123',
    deactivatedByRole: 'admin',
    notes: 'Staff resigned for personal reasons'
  };
  ...
  describe('execute - successful deactivation', () => {
    it('should deactivate staff with valid data', async () => {
      ...
      expect(result.data?.reason).toBe(validRequest.reason);
      expect(mockStaffRepository.save).toHaveBeenCalledWith(existingStaff);
    });

    it('should deactivate with effective date', async () => {
      mockStaffRepository.findById.mockResolvedValue(existingStaff);
      mockStaffRepository.save.mockResolvedValue(undefined);
    ...
    it('should log deactivation for audit', async () => {
      mockStaffRepository.findById.mockResolvedValue(existingStaff);
      mockStaffRepository.save.mockResolvedValue(undefined);
      ...
      expect(mockLogger.info).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          ...
          deactivatedBy: validRequest.deactivatedBy
        })
      );
    });
  });
  ...
  describe('execute - validation errors', () => {
    ...
    it('should fail when reason is empty', async () => {
      ...
      expect(result.success).toBe(false);
      expect(result.errors).toContain('Lý do deactivate là bắt buộc');
    });

    it('should fail when deactivatedBy is missing', async () => {
      const invalidRequest = {
        ...validRequest,
        deactivatedBy: ''
      };
  ...
  describe('execute - authorization', () => {
    it('should allow admin to deactivate staff', async () => {
      mockStaffRepository.findById.mockResolvedValue(existingStaff);
      mockStaffRepository.save.mockResolvedValue(undefined);
    ...
    it('should fail when non-admin tries to deactivate', async () => {
      mockStaffRepository.findById.mockResolvedValue(existingStaff);
  ...
  describe('execute - error handling', () => {
    ...
    it('should handle invalid effective date', async () => {
      mockStaffRepository.findById.mockResolvedValue(existingStaff);
  ...
  describe('HIPAA audit logging', () => {
    it('should log deactivation for HIPAA compliance', async () => {
      mockStaffRepository.findById.mockResolvedValue(existingStaff);
      mockStaffRepository.save.mockResolvedValue(undefined);
