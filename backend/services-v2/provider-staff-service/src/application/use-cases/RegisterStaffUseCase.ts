 ...
 */
export class RegisterStaffUseCase extends BaseHealthcareUseCase<
...
> {
  ...
  protected async executeImpl(
    ...
    try {
      ...
      const duplicateStaff = await this.staffRepository.findByLicenseNumber(
  ...
  protected override async validateRequest(
    ...
    if (
      ...
      request.personalInfo.fullName.trim().length === 0
    ) {
    ...
    if (
      ...
      request.personalInfo.nationalId.trim().length === 0
    ) {
    ...
    if (
      ...
      request.personalInfo.phoneNumber.trim().length === 0
    ) {
    ...
    if (
      !request.professionalInfo.title ||
      request.professionalInfo.title.trim().length === 0
    ) {
    ...
    if (
      !request.professionalInfo.department ||
      request.professionalInfo.department.trim().length === 0
    ) {
   ...
   */
  private async auditStaffRegistration(
    staff: ProviderStaff,
    request: RegisterStaffRequest,
  ): Promise<void> {
    this.logger.info("HIPAA Audit: Staff registration", {
      ...
      dataAccessed:
        "staff_personal_info,staff_professional_info,staff_credentials",
