...
/**
 * Circuit Breaker for Provider Staff Operations
 * Implements fail-fast pattern to prevent system overload
 */
export class ProviderStaffCircuitBreaker {
...
export class CircuitBreakerFactory {
  private static breakers: Map<string, ProviderStaffCircuitBreaker> = new Map();

  static getBreaker(
    serviceName: string,
    config?: Partial<CircuitBreakerConfig>
  ): ProviderStaffCircuitBreaker {
    if (!this.breakers.has(serviceName)) {
      ...
      this.breakers.set(serviceName, new ProviderStaffCircuitBreaker(finalConfig, serviceName));
    }

    return this.breakers.get(serviceName)!;
  }

  static getAllBreakers(): Map<string, ProviderStaffCircuitBreaker> {
    return this.breakers;
  }

  static getHealthStatus() {
    const status: Record<string, ReturnType<ProviderStaffCircuitBreaker['getStatus']>> = {};
    let openBreakers = 0;
...
export { ProviderStaffCircuitBreaker as CircuitBreaker };
