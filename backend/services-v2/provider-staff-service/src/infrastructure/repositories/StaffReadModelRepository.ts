...
export interface IStaffReadModelRepository {
  ...
  updateRating(staffId: string, props: StaffReadModelUpdateProps): Promise<void>;
...
export class StaffReadModelRepository implements IStaffReadModelRepository {
  ...
  async findById(staffId: string): Promise<StaffReadModel | null> {
    try {
      ...
      this.logger.error('Error finding staff read model by ID', { staffId, error });
      throw error;
    }
  }

  async findAll(limit: number = 50, offset: number = 0): Promise<StaffReadModel[]> {
    try {
      const { data, error } = await this.supabaseClient
        .from(`${this.schema}.${this.tableName}`)
        .select('*')
      ...
      if (error) {
        throw new Error(`Failed to find all staff read models: ${error.message}`);
      }
  ...
  async findByDepartment(department: string): Promise<StaffReadModel[]> {
    try {
      ...
      this.logger.error('Error finding staff by department', { department, error });
      throw error;
    }
  }
  ...
  async create(props: StaffReadModelCreateProps): Promise<void> {
    try {
      ...
      if (error) {
        throw new Error(`Failed to create staff read model: ${error.message}`);
      }
  ...
  async updateRating(staffId: string, props: StaffReadModelUpdateProps): Promise<void> {
    try {
      ...
      this.logger.error('Error updating staff read model', { staffId, props, error });
      throw error;
    }
  }
