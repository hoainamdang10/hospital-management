...
export class RabbitMQStaffEventHandler implements IEventHandler<DomainEvent> {
  ...
  async handleStaffUpdated(event: DomainEvent): Promise<void> {
    try {
      ...
      if (
        eventData.updatedFields.includes("workSchedule") &&
        updatedData.workSchedule
      ) {
      ...
      if (
        eventData.updatedFields.includes("consultationFee") &&
        updatedData.consultationFee !== undefined
      ) {
    ...
    try {
      ...
      const integrationEvent = createDoctorAvailabilityChangedEvent({
        ...
        effectiveDate: eventData.effectiveDate
          ? new Date(eventData.effectiveDate)
          : undefined,
      });
      ...
      this.logger.info(
        "DoctorAvailabilityChanged event processed successfully",
        {
          staffId: eventData.staffId,
          eventId: event.eventId,
        },
  ...
  async handleStaffEmploymentStatusUpdated(event: DomainEvent): Promise<void> {
    try {
      ...
      this.logger.info(
        "StaffEmploymentStatusUpdated event processed successfully",
        {
          staffId: eventData.staffId,
          eventId: event.eventId,
        },
