...
export class AuthenticateRequestUseCase {
  ...
  async execute(input: AuthenticateRequestInput): Promise<AuthenticateRequestOutput> {
    try {
      ...
      const verificationResult: TokenVerificationResult = await this.tokenVerifier.verify(token);
