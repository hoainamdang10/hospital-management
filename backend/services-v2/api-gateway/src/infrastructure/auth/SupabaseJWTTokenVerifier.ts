...
export class SupabaseJWTTokenVerifier implements ITokenVerifier {
  ...
  private validateConfig(): void {
    ...
    if (
      ...
      this.config.jwtSecret !== process.env.SUPABASE_JWT_SECRET
    ) {
  ...
  async verify(token: JWTToken): Promise<TokenVerificationResult> {
    try {
      const {
        data: { user },
        error,
      } = await this.supabaseClient.auth.getUser(token.value);
      ...
      const normalizeStrings = (values: unknown[], toLower = false) =>
        values
      ...
      const permissions = normalizeStrings(toArray(rawPermissions), false);

      const patientId =
        user.user_metadata?.patientId ||
        user.user_metadata?.patient_id ||
        user.app_metadata?.patientId ||
        user.app_metadata?.patient_id;

      const providerId =
        user.user_metadata?.providerId ||
