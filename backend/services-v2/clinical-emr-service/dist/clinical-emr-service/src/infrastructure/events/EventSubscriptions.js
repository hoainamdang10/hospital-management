...
class EventSubscriptions {
    ...
    async setupSubscriptions() {
        ...
        await this.eventBus.subscribe('AppointmentCompleted', {
            handle: async (event) => {
                this.logger.info(`[Event] 📅 Received AppointmentCompleted: ${event.aggregateId}`, event.metadata);
        ...
        await this.eventBus.subscribe('AppointmentCancelled', {
            handle: async (event) => {
                this.logger.info(`[Event] ❌ Received AppointmentCancelled: ${event.aggregateId}`, event.metadata);
        ...
        await this.eventBus.subscribe('PatientRegistered', {
            handle: async (event) => {
                this.logger.info(`[Event] 👤 Received PatientRegistered: ${event.aggregateId}`, event.metadata);
                await this.clinicalEMREventHandler.handleEvent(event);
            }
        ...
        await this.eventBus.subscribe('PatientUpdated', {
            handle: async (event) => {
                this.logger.info(`[Event] 👤 Received PatientUpdated: ${event.aggregateId}`, event.metadata);
        ...
        await this.eventBus.subscribe('StaffRegistered', {
            handle: async (event) => {
                this.logger.info(`[Event] 👨‍⚕️ Received StaffRegistered: ${event.aggregateId}`, event.metadata);
        ...
        await this.eventBus.subscribe('StaffUpdated', {
            handle: async (event) => {
                this.logger.info(`[Event] 👨‍⚕️ Received StaffUpdated: ${event.aggregateId}`, event.metadata);
        ...
        await this.eventBus.subscribe('TestResultsReady', {
            handle: async (event) => {
                this.logger.info(`[Event] 🧪 Received TestResultsReady: ${event.aggregateId}`, event.metadata);
                await this.clinicalEMREventHandler.handleEvent(event);
            }
        ...
        await this.eventBus.subscribe('PaymentCompleted', {
            handle: async (event) => {
                this.logger.info(`[Event] 💰 Received PaymentCompleted: ${event.aggregateId}`, event.metadata);
        ...
        await this.eventBus.subscribe('MedicalRecordCreated', {
            handle: async (event) => {
                this.logger.info(`[Event] 📋 Received MedicalRecordCreated: ${event.aggregateId}`, event.metadata);
        ...
        await this.eventBus.subscribe('MedicalRecordUpdated', {
            handle: async (event) => {
                this.logger.info(`[Event] 📝 Received MedicalRecordUpdated: ${event.aggregateId}`, event.metadata);
                await this.medicalRecordDomainEventHandler.handle(event);
            }
