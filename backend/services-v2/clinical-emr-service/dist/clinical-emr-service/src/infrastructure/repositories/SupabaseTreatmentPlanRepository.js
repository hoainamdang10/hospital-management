...
class SupabaseTreatmentPlanRepository {
    constructor(supabase) {
        ...
        this.tableName = 'treatment_plans';
    ...
    async findByMedicalRecordId(medicalRecordId, options) {
        ...
        if (error)
            throw new Error(`Failed to find plans by medical record: ${error.message}`);
    ...
    async findByPatientId(patientId, options) {
        ...
        const { data, error } = await query;
        if (error)
            throw new Error(`Failed to find plans by patient: ${error.message}`);
        return data ? data.map(r => this.toDomain(r)) : [];
    }
    async findByCreatedBy(doctorId, options) {
        let query = this.supabase
            .schema(this.schema)
            .from(this.tableName)
            .select('*')
        ...
        const { data, error } = await query;
        if (error)
            throw new Error(`Failed to find plans by doctor: ${error.message}`);
        return data ? data.map(r => this.toDomain(r)) : [];
    }
    async findByStatus(status, options) {
        ...
        const { data, error } = await query;
        if (error)
            throw new Error(`Failed to find plans by status: ${error.message}`);
        return data ? data.map(r => this.toDomain(r)) : [];
    }
    async findActivePlansByPatient(patientId) {
        const { data, error } = await this.supabase
            .schema(this.schema)
            .from(this.tableName)
            .select('*')
    ...
    async findPendingConsentPlans(options) {
        let query = this.supabase
            .schema(this.schema)
            .from(this.tableName)
            .select('*')
        ...
        const { data, error } = await query;
        if (error)
            throw new Error(`Failed to find pending consent plans: ${error.message}`);
        return data ? data.map(r => this.toDomain(r)) : [];
    }
    async search(filters) {
        ...
        if (filters.fromDate)
            query = query.gte('start_date', filters.fromDate.toISOString());
        if (filters.toDate)
            query = query.lte('end_date', filters.toDate.toISOString());
        if (filters.diagnosisCode)
            query = query.eq('diagnosis_code', filters.diagnosisCode);
        if (filters.hasConsent !== undefined)
            query = query.eq('patient_consent_granted', filters.hasConsent);
        if (filters.searchText)
            query = query.or(`diagnosis.ilike.%${filters.searchText}%,notes.ilike.%${filters.searchText}%`);
        ...
        const { data, error } = await query;
        if (error)
            throw new Error(`Failed to search plans: ${error.message}`);
        return data ? data.map(r => this.toDomain(r)) : [];
    }
    async count(filters) {
    ...
    async delete(planId) {
        const { error } = await this.supabase
            ...
            .eq('plan_id', planId.value);
        if (error)
            throw new Error(`Failed to delete treatment plan: ${error.message}`);
    }
    async exists(planId) {
        const { count, error } = await this.supabase
            ...
            .eq('plan_id', planId.value);
        if (error)
            throw new Error(`Failed to check plan existence: ${error.message}`);
        return (count || 0) > 0;
    }
    ...
    async findByDateRange(startDate, endDate, options) {
        ...
        const { data, error } = await query;
        if (error)
            throw new Error(`Failed to find plans by date range: ${error.message}`);
        return data ? data.map(r => this.toDomain(r)) : [];
    }
    toDatabase(plan) {
        return {
    ...
    toDomain(record) {
        const planId = TreatmentPlanId_1.TreatmentPlanId.create(record.plan_id);
        const props = {
            planId,
        ...
        return TreatmentPlan_aggregate_1.TreatmentPlanAggregate.reconstitute(props, record.plan_id);
    }
}
exports.SupabaseTreatmentPlanRepository = SupabaseTreatmentPlanRepository;
//# sourceMappingURL=SupabaseTreatmentPlanRepository.js.map