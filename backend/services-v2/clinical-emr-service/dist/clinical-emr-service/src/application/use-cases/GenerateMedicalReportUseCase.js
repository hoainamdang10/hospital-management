    ...
    generatePrescriptionReport(medicalRecord, request) {
        ...
        const report = {
            ...
            medications: activeMedications.map((m, index) => ({
                ...
                strength: m.strength,
                dosageForm: m.dosageForm || m.toJSON().dosageForm,
                dosage: m.dosage || m.toJSON().dosage,
                frequency: m.frequency || m.toJSON().frequency,
                duration: m.duration || m.toJSON().duration,
                instructions: m.instructions || m.toJSON().instructions,
                specialInstructions: m.specialInstructions || m.toJSON().specialInstructions,
                vietnameseSummary: m.getVietnameseSummary()
            })),
