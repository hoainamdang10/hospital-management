 ...
 */
class NoteId extends value_object_1.ValueObject {
    constructor(props) {
        super(props);
    }
    /**
    ...
    static create(value) {
        ...
        if (!this.FORMAT_REGEX.test(trimmedValue)) {
            throw new Error(`NoteId phải có định dạng NOTE-YYYYMM-XXX (e.g., NOTE-202501-001), nhận được: ${trimmedValue}`);
        }
    ...
    validateFormat() {
        if (!NoteId.FORMAT_REGEX.test(this.props.value)) {
            throw new Error(`NoteId không hợp lệ: ${this.props.value}. Định dạng yêu cầu: NOTE-YYYYMM-XXX`);
        }
...
NoteId.FORMAT_REGEX = /^NOTE-\d{6}-\d{3}$/;
