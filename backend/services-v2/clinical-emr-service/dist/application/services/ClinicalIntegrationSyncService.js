...
exports.ClinicalIntegrationSyncService = void 0;
class ClinicalIntegrationSyncService {
    constructor(patientRepo, providerRepo, logger) {
    ...
    async syncPatientSnapshot(payload, eventType) {
    ...
    async syncAppointmentContext(payload, eventType) {
        const patientId = payload.patientId ||
            payload.eventData?.patientId ||
            payload.data?.patientId ||
            payload.patient?.patientId;
        if (!patientId) {
            this.logger.warn("[IntegrationSync] Appointment event missing patientId", {
                eventType,
                payload,
            });
            return;
    ...
    async syncProviderSnapshot(payload, isActive, eventType) {
        ...
        if (!isActive) {
            await this.providerRepo.deactivate(provider.providerId, eventType ?? "provider.staff.deactivated");
            return;
        }
    ...
    extractPatientPayload(payload) {
        const data = payload.eventData ?? payload.payload ?? payload.data ?? payload;
        ...
        return {
            ...
            dateOfBirth: data.dateOfBirth ?? personalInfo.dateOfBirth ?? data.dob ?? null,
            gender: data.gender ?? personalInfo.gender ?? data.sex ?? null,
            phone: pickString(data.phoneNumber, data.phone, personalInfo.phoneNumber, contactInfo.primaryPhone, contactInfo.phone, contactInfo.phoneNumber),
            email: pickString(data.email, data.contactEmail, personalInfo.email, contactInfo.email),
            address,
            insurance,
    ...
    extractProviderPayload(payload) {
        const data = payload.payload ?? payload.eventData ?? payload.data ?? payload;
...
exports.ClinicalIntegrationSyncService = ClinicalIntegrationSyncService;
