...
class CreateMedicalRecordUseCase extends use_case_interface_1.BaseHealthcareUseCase {
    ...
    async executeInternal(request) {
        try {
            ...
            const medicalRecord = clinical_aggregate_1.MedicalRecordAggregate.create(recordId, request.patientId, request.doctorId, new Date(request.visitDate), request.createdBy, request.appointmentId, request.symptoms, request.examinationNotes, request.diagnosis, request.treatment, request.medications, request.notes, vitalSigns);
            // Save to repository
