...
import { 
  ...
  validateUpdateMedicalRecordRequest,
  extractUpdateFields,
  hasVitalSignsUpdate
...
export class UpdateMedicalRecordUseCase extends BaseHealthcareUseCase<UpdateMedicalRecordRequest, UpdateMedicalRecordResponse> {
   ...
   */
  protected async executeInternal(request: UpdateMedicalRecordRequest): Promise<UpdateMedicalRecordResponse> {
    try {
      ...
      const medicalUpdates = extractUpdateFields(request);
      ...
      if (hasVitalSignsUpdate(request)) {
        ...
        const newVitalSigns = BasicVitalSigns.create(newVitalSignsData);
   ...
   * Check authorization
   */
  async authorize(request: UpdateMedicalRecordRequest, userId: string): Promise<boolean> {
    try {
   ...
   * Validate business rules
   */
  private async validateBusinessRules(request: UpdateMedicalRecordRequest): Promise<string[]> {
   ...
   */
  async authorizeWithRoles(request: UpdateMedicalRecordRequest, userId: string, userRoles: string[]): Promise<boolean> {
   ...
   */
  async getAuditInfoAsync(request: UpdateMedicalRecordRequest) {
    ...
    return {
      ...
      details: {
        recordId: request.recordId,
        updatedFields: Object.keys(extractUpdateFields(request)),
        hasVitalSignsUpdate: hasVitalSignsUpdate(request),
        updatedBy: request.updatedBy,
        updateReason: request.updateReason
      },
