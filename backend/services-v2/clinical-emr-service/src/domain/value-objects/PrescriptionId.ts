 ...
 */
export class PrescriptionId extends ValueObject<PrescriptionIdProps> {
  private static readonly FORMAT_REGEX = /^PRESC-\d{6}-\d{3}$/;
  
  private constructor(props: PrescriptionIdProps) {
    super(props);
  }
   ...
   */
  public static create(value: string): PrescriptionId {
    if (!value || value.trim() === '') {
      throw new Error('PrescriptionId không được để trống');
    }
    ...
    if (!this.FORMAT_REGEX.test(trimmedValue)) {
      throw new Error(
        `PrescriptionId phải có định dạng PRESC-YYYYMM-XXX (e.g., PRESC-202501-001), nhận được: ${trimmedValue}`
      );
    }

    return new PrescriptionId({ value: trimmedValue });
  }

  /**
   * Generate new PrescriptionId with current date
   */
  public static generate(sequenceNumber: number): PrescriptionId {
    const now = new Date();
    const year = now.getFullYear();
    const month = (now.getMonth() + 1).toString().padStart(2, '0');
    const sequence = sequenceNumber.toString().padStart(3, '0');
    
    const prescriptionId = `PRESC-${year}${month}-${sequence}`;
    return new PrescriptionId({ value: prescriptionId });
  }

  protected validateFormat(): void {
    if (!PrescriptionId.FORMAT_REGEX.test(this.props.value)) {
      throw new Error(
        `PrescriptionId không hợp lệ: ${this.props.value}. Định dạng yêu cầu: PRESC-YYYYMM-XXX`
      );
    }
    ...
    if (year < 2020 || year > currentYear + 1) {
      throw new Error(`Năm không hợp lệ trong PrescriptionId: ${year}`);
    }

    if (month < 1 || month > 12) {
      throw new Error(`Tháng không hợp lệ trong PrescriptionId: ${month}`);
    }

    // Validate sequence number
    const sequence = parseInt(parts[2], 10);
    if (sequence < 1 || sequence > 999) {
      throw new Error(`Số thứ tự không hợp lệ trong PrescriptionId: ${sequence}`);
    }
  }
