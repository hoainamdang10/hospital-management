...
export class AdvancedSearchService {
  ...
  private searchCache: Map<string, { results: any; timestamp: Date }> = new Map();
  ...
  async search(
    ...
    try {
      ...
      if (criteria.searchText && criteria.searchText.trim() !== '') {
        filteredRecords = await this.performTextSearch(
          filteredRecords,
          criteria.searchText,
          options
        );
      ...
      const resultsWithRelevance = await this.calculateRelevanceScores(
        filteredRecords,
        criteria,
        options
      );
   ...
   */
  private async updateSearchIndex(records: MedicalRecordAggregate[]): Promise<void> {
  ...
  private applyAdvancedFilters(
    ...
    return records.filter(record => {
      ...
      if (criteria.diagnosisCode) {
        const hasDiagnosis = record.diagnoses.some(d => 
          d.code.toLowerCase().includes(criteria.diagnosisCode!.toLowerCase()) ||
      ...
      if (criteria.medicationCode) {
        const hasMedication = record.medications.some(m => 
          m.code.toLowerCase().includes(criteria.medicationCode!.toLowerCase()) ||
  ...
  private async calculateRelevanceScores(
    ...
    criteria: SearchCriteria,
    options: AdvancedSearchOptions
  ): Promise<SearchResultWithRelevance[]> {
    ...
    for (const record of records) {
      ...
      if (criteria.searchText) {
        ...
        for (const term of expandedTerms) {
          ...
          // Check diagnosis
          if (record.diagnoses.some(d => 
            d.display.toLowerCase().includes(term.toLowerCase()) ||
   ...
   */
  private extractSearchableContent(record: MedicalRecordAggregate): string {
    const parts: string[] = [];

    if (record.symptoms) parts.push(record.symptoms);
    if (record.examinationNotes) parts.push(record.examinationNotes);
   ...
   */
  private extractKeywords(record: MedicalRecordAggregate): string[] {
    ...
    record.diagnoses.forEach(d => {
      ...
      if (d.category) keywords.push(d.category);
    });
    ...
    record.medications.forEach(m => {
      ...
      if (m.genericName) keywords.push(m.genericName);
    });

    // Add status and other metadata
    keywords.push(record.status);
    if (record.specialtyCode) keywords.push(record.specialtyCode);
   ...
   */
  private extractVietnameseTerms(record: MedicalRecordAggregate): string[] {
    const terms: string[] = [];
    ...
    Object.keys(VIETNAMESE_MEDICAL_TERMS).forEach(vietnameseTerm => {
      if (content.includes(vietnameseTerm)) {
        ...
        terms.push(...VIETNAMESE_MEDICAL_TERMS[vietnameseTerm as keyof typeof VIETNAMESE_MEDICAL_TERMS]);
      }
    });
   ...
   */
  private expandVietnameseTerms(terms: string[], language?: 'vi' | 'en' | 'both'): string[] {
    if (language === 'en') {
   ...
   */
  private fuzzyMatch(term: string, content: string, threshold: number = 0.8): boolean {
   ...
   */
  private generateHighlightedContent(record: MedicalRecordAggregate, matchedTerms: string[]): string {
   ...
   * Generate cache key
   */
  private generateCacheKey(criteria: SearchCriteria, options: AdvancedSearchOptions): string {
