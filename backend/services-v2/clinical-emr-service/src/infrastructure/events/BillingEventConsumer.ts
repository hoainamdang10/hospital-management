...
export interface InvoicePaidEventData {
  ...
  items: Array<{
    type: 'consultation' | 'medication' | 'lab_test' | 'procedure' | 'imaging';
    description: string;
    quantity: number;
    unitPrice: number;
    totalPrice: number;
...
export interface InvoiceFinalizedEventData {
  ...
  currency: string;
  finalizedAt: Date;
  status: 'finalized' | 'pending_payment' | 'paid' | 'overdue';
  insuranceCoverage?: {
    providerName: string;
    policyNumber: string;
...
export class BillingEventConsumer {
    ...
    try {
      ...
      switch (routingKey) {
        case 'billing.invoice.paid':
          await this.handleInvoicePaid(event.payload as InvoicePaidEventData);
          break;

        case 'billing.invoice.finalized':
          await this.handleInvoiceFinalized(event.payload as InvoiceFinalizedEventData);
          break;

        case 'billing.insurance.claim.processed':
          await this.handleInsuranceClaimProcessed(event.payload as InsuranceClaimProcessedEventData);
          break;
   ...
   */
  private async handleInvoicePaid(data: InvoicePaidEventData): Promise<void> {
    ...
    try {
      ...
      if (data.medicalRecordId) {
        const getMedicalRecordUseCase = new GetMedicalRecordUseCase(this.medicalRecordRepo);
   ...
   */
  private async handleInvoiceFinalized(data: InvoiceFinalizedEventData): Promise<void> {
   ...
   */
  private async handleInsuranceClaimProcessed(data: InsuranceClaimProcessedEventData): Promise<void> {
