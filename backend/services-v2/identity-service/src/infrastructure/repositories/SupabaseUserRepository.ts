...
export class SupabaseUserRepository implements IUserRepository {
  ...
  async createAuthUser(userData: CreateAuthUserData): Promise<User> {
    return await this.circuitBreaker.execute(async () => {
      ...
      if (authError || !authUser.user) {
        ...
        throw new Error(
          `Failed to create auth user: ${getErrorMessage(authError)}`,
        );
      }
      ...
      if (profileError || !profile) {
        ...
        throw new Error(
          `Failed to create user profile: ${getErrorMessage(profileError)}`,
        );
      }
      ...
      if (this.permissionRepository) {
        ...
        try {
          ...
          this.logger.error(
            "Failed to get valid roles from database, using fallback",
        ...
        if (!validRoles.includes(userData.roleType.toLowerCase())) {
          ...
          await this.supabaseClient
            ...
            .eq("id", profile.id);
          throw new Error(
            `Invalid role type: ${userData.roleType}. Valid roles: ${validRoles.join(", ")}`,
          );
        }
        ...
        try {
          ...
          this.logger.error(
            "Failed to assign role in Pure RBAC, rolling back user creation",
            {
              userId: profile.id,
              role: userData.roleType,
  ...
  async update(user: User): Promise<void> {
    return await this.circuitBreaker.execute(async () => {
      ...
      for (const key of Object.keys(updateRecord) as Array<
        keyof typeof updateRecord
      >) {
  ...
  async updateAuthEmailConfirmed(userId: UserId): Promise<void> {
    ...
    try {
      ...
      if (error) {
        throw new Error(
          `Failed to update auth email confirmed: ${getErrorMessage(error)}`,
        );
      }
  ...
  async exists(userId: UserId): Promise<boolean> {
    ...
    if (error && error.code !== "PGRST116") {
      throw new Error(
        `Failed to check user existence: ${getErrorMessage(error)}`,
      );
    }
    return !!data;
  }
  ...
  async invalidateSession(
    ...
    return await this.circuitBreaker.execute(async () => {
      ...
      if (error) {
        throw new Error(
          `Failed to invalidate session: ${getErrorMessage(error)}`,
        );
      }
  ...
  async getUserRoles(userId: UserId): Promise<string[]> {
    ...
    return await this.circuitBreaker.execute(
      async () => {
        ...
        if (error) {
          throw new Error(
            `Failed to get user roles: ${getErrorMessage(error)}`,
          );
        }

        // Extract role names from user_roles table
        const roles =
          data?.map((row: { role_name: string }) => row.role_name) || [];
  ...
  private async mapToUserAggregate(data: UserRecord): Promise<User> {
    ...
    if (this.permissionRepository) {
      try {
        ...
        this.logger.warn(
          "Failed to load user roles from Pure RBAC, falling back to role_type",
          {
            userId: data.id,
            error: getErrorMessage(error),
          },
  ...
  async disableMFA(userId: UserId): Promise<void> {
    ...
    try {
      ...
      if (profileError) {
        throw new Error(
          `Failed to update user profile: ${getErrorMessage(profileError)}`,
        );
      }
  ...
  async checkAccountLockout(
    email: Email,
  ): Promise<{ isLocked: boolean; unlockAt?: Date; failedAttempts: number }> {
    try {
      const emailValue = email.value;
      ...
      if (failedAttempts >= 5) {
        ...
        const unlockAt = new Date(
          new Date(firstFailedAttempt.attempted_at).getTime() + 30 * 60 * 1000,
        ); // Fixed: Use 'attempted_at'
  ...
  async getUserPermissions(userId: UserId): Promise<string[]> {
    ...
    this.logger.warn(
      "getUserPermissions() called on UserRepository. Use IPermissionRepository instead.",
      {
        userId: userId.value,
      },
    );
  ...
  async getActiveSessions(userId: UserId): Promise<UserSession[]> {
    ...
    if (error) {
      throw new Error(
        `Failed to get active sessions: ${getErrorMessage(error)}`,
      );
    }
  ...
  async list(options?: {
    ...
    if (options?.filters) {
      ...
      Object.entries(otherFilters).forEach(([key, value]) => {
        if (!allowedFilterKeys.includes(key)) {
          throw new Error(
            `Invalid filter key: ${key}. Allowed keys: ${allowedFilterKeys.join(", ")}`,
          );
        }
      ...
      if (search_term) {
        ...
        query = query.or(
          `full_name.ilike.%${escapedTerm}%,email.ilike.%${escapedTerm}%`,
        );
      }
    }
    ...
    return await Promise.all(
      data.map((record) => this.mapToUserAggregate(record)),
    );
  }
  ...
  async storeStaffInvitation(data: {
    ...
    try {
      ...
      if (error) {
        throw new Error(
          `Failed to store staff invitation: ${getErrorMessage(error)}`,
        );
      }
  ...
  async markInvitationAsUsed(token: string, userId: string): Promise<void> {
    try {
      ...
      if (error) {
        throw new Error(
          `Failed to mark invitation as used: ${getErrorMessage(error)}`,
        );
      }
  ...
  async listStaffInvitations(options?: {
    ...
    try {
      ...
      if (error) {
        throw new Error(
          `Failed to list staff invitations: ${getErrorMessage(error)}`,
        );
      }
  ...
  async cancelStaffInvitation(id: string, cancelledBy: string): Promise<void> {
    try {
      ...
      if (error) {
        throw new Error(
          `Failed to cancel staff invitation: ${getErrorMessage(error)}`,
        );
      }
  ...
  async resendStaffInvitation(id: string): Promise<{
    ...
    try {
      ...
      if (error) {
        throw new Error(
          `Failed to resend staff invitation: ${getErrorMessage(error)}`,
        );
      }
