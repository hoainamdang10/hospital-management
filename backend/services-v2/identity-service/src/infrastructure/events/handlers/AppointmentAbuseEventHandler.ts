...
export class AppointmentAbuseEventHandler {
    ...
    try {
      ...
      if (event.hoursNotice < 24) {
        const lastMinuteCancellations =
          await this.getLastMinuteCancellationCount(event.patientId, 30);
        ...
        if (lastMinuteCancellations >= 3) {
          ...
          this.logger.info(
            "User flagged for review due to cancellation pattern",
            {
              patientId: event.patientId,
  ...
  private async getNoShowCount(userId: string, days: number): Promise<number> {
    ...
    if (error) {
      this.logger.error("Error getting no-show count", {
        error: error.message,
      });
      return 0;
    }
   ...
   */
  private async getLastMinuteCancellationCount(
    userId: string,
    days: number,
  ): Promise<number> {
    const cutoffDate = new Date();
    ...
    if (error) {
      this.logger.error("Error getting cancellation count", {
        error: error.message,
      });
      return 0;
    }
  ...
  async handleAppointmentRescheduled(
    ...
    try {
      ...
      const rescheduleCount = await this.getRescheduleCount(
        event.patientId,
        30,
      );
   ...
   */
  async handleAppointmentLateArrival(
    event: AppointmentLateArrivalEvent,
  ): Promise<void> {
    try {
      // Idempotency check
      const processed = await this.inboxService.checkProcessed(event.eventId);
      ...
      const lateArrivalCount = await this.getLateArrivalCount(
        event.patientId,
        30,
      );
      ...
      this.logger.info(
        "Successfully processed appointment.late_arrival event",
        {
          eventId: event.eventId,
          patientId: event.patientId,
   ...
   */
  private async getRescheduleCount(
    userId: string,
    days: number,
  ): Promise<number> {
    const cutoffDate = new Date();
    ...
    if (error) {
      this.logger.error("Error getting reschedule count", {
        error: error.message,
      });
      return 0;
    }
   ...
   */
  private async getLateArrivalCount(
    userId: string,
    days: number,
  ): Promise<number> {
    const cutoffDate = new Date();
    ...
    if (error) {
      this.logger.error("Error getting late arrival count", {
        error: error.message,
      });
      return 0;
    }
  ...
  async handleAppointmentCompleted(
    ...
    try {
      ...
      if (event.wasOnTime && event.hadNoIssues) {
        ...
        const hasRestrictions = await this.hasActiveRestrictions(
          event.patientId,
        );

        if (hasRestrictions) {
          // Get count of recent completed appointments with good behavior
          const goodBehaviorCount = await this.getGoodBehaviorCount(
            event.patientId,
            30,
          );
          ...
          if (goodBehaviorCount >= 3) {
            ...
            this.logger.info(
              "Booking restrictions reset due to good behavior",
              {
                patientId: event.patientId,
                goodBehaviorCount,
                eventId: event.eventId,
            ...
            this.logger.debug(
              "Good behavior tracked, but not enough to reset restrictions",
              {
                patientId: event.patientId,
                goodBehaviorCount,
                requiredCount: 3,
   ...
   */
  private async getGoodBehaviorCount(
    userId: string,
    days: number,
  ): Promise<number> {
    try {
