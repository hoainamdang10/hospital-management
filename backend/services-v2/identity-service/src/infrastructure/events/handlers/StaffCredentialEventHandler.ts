...
export class StaffCredentialEventHandler {
   ...
   */
  async handleStaffCredentialVerified(
    event: StaffCredentialVerifiedEvent,
  ): Promise<void> {
    try {
      // Idempotency check
      ...
      this.logger.info(
        "Staff credential verified - user activation skipped (requires staff-user mapping)",
        {
          staffId: event.staffId,
   ...
   */
  async handleStaffStatusChanged(
   ...
   */
  async handleStaffCredentialExpired(
    event: StaffCredentialExpiredEvent,
  ): Promise<void> {
    try {
      // Idempotency check
      const processed = await this.inboxService.checkProcessed(event.eventId);
   ...
   */
  async handleStaffLicenseRevoked(
    event: StaffLicenseRevokedEvent,
  ): Promise<void> {
    try {
      // Idempotency check
      const processed = await this.inboxService.checkProcessed(event.eventId);
      ...
      this.logger.error(
        "User account locked and sessions terminated due to license revocation",
        {
          userId,
          licenseNumber: event.licenseNumber,
          revocationReason: event.revocationReason,
   ...
   */
  async handleStaffPerformanceFlagged(
    event: StaffPerformanceFlaggedEvent,
  ): Promise<void> {
    try {
      // Idempotency check
      const processed = await this.inboxService.checkProcessed(event.eventId);
      ...
      this.logger.info(
        "Successfully processed staff.performance_flagged event",
        {
          eventId: event.eventId,
          staffId: event.staffId,
          actionTaken:
            event.severity === "CRITICAL" ? "ACCOUNT_LOCKED" : "FLAGGED",
        },
      );
    } catch (error) {
   ...
   */
  async handleStaffDepartmentChanged(
    event: StaffDepartmentChangedEvent,
  ): Promise<void> {
    try {
      // Idempotency check
      const processed = await this.inboxService.checkProcessed(event.eventId);
      ...
      this.logger.info(
        "Successfully processed staff.department_changed event",
        {
          eventId: event.eventId,
          userId: event.userId,
          newDepartment: event.newDepartmentName,
   ...
   */
  async handleStaffScheduleUpdated(
