...
export class IdentityServiceDegradation implements IDegradationService {
   ...
   */
  private async fallbackAuthentication(
    credentials: UserCredentials,
  ): Promise<AuthResult> {
   ...
   */
  private async readOnlyAuthentication(
    credentials: UserCredentials,
  ): Promise<AuthResult> {
    ...
    if (!entry) {
      throw new Error(
        "No cached credentials available - authentication failed",
      );
    }
   ...
   */
  private async emergencyAuthentication(
    credentials: UserCredentials,
  ): Promise<AuthResult> {
    ...
    if (!entry) {
      this.logger.error(
        "SECURITY ALERT: Emergency access denied - no cached credentials",
        {
          email: credentials.email,
  ...
  public async cacheAuthentication(
    ...
    if (!plaintextPassword || plaintextPassword.length === 0) {
      this.logger.warn(
        "Skipping authentication cache due to missing password",
        { email },
      );
      return;
    }
  ...
  private isHealthcareStaffEmail(email: string): boolean {
    ...
    return healthcareDomains.includes(domain) || email.includes("doctor");
  }
