...
export function createAuthRoutes(deps: RouteDependencies): Router {
  ...
  router.post("/forgot-password", async (req, res) => {
    try {
      ...
      const result = await deps.forgotPasswordUseCase.execute({ email });
  ...
  router.get("/verify-email", async (req, res) => {
    try {
      ...
      logger.error("Verify email GET endpoint error", {
        error: getErrorMessage(error),
      });
      return res.status(500).json({
        success: false,
  ...
  router.post("/verify-email", async (req, res) => {
    try {
      ...
      logger.error("Verify email POST endpoint error", {
        error: getErrorMessage(error),
      });
      return res.status(500).json({
        success: false,
  ...
  router.post("/resend-verification", async (req, res) => {
    try {
      ...
      logger.error("Resend verification email endpoint error", {
        error: getErrorMessage(error),
      });
      return res.status(500).json({
        success: false,
  ...
  router.get("/validate-invitation", async (req, res) => {
    try {
      ...
      logger.error("Validate staff invitation endpoint error", {
        error: getErrorMessage(error),
      });
      return res.status(500).json({
        success: false,
  ...
  router.post("/activate-staff", async (req, res) => {
    try {
      ...
      logger.error("Accept staff invitation endpoint error", {
        error: getErrorMessage(error),
      });
      res.status(500).json({
        success: false,
      ...
      try {
        ...
        return res.status(200).json({
          ...
          user: {
            ...
            fullName:
              req.user.fullName || req.user.email?.split("@")[0] || "User",
