...
export class ClinicalComplianceEventHandler {
   ...
   */
  async handleMedicalRecordFlagged(event: MedicalRecordFlaggedEvent): Promise<void> {
    try {
      // Idempotency check
      ...
      if (event.severity === 'CRITICAL' || event.severity === 'HIGH') {
        ...
        this.logger.error('User account locked due to medical record violation', {
          userId,
          recordId: event.recordId,
          severity: event.severity,
          flagReason: event.flagReason
   ...
   */
  async handlePrescriptionAbuseDetected(event: PrescriptionAbuseDetectedEvent): Promise<void> {
    try {
      // Idempotency check
      const processed = await this.inboxService.checkProcessed(event.eventId);
      if (processed) {
