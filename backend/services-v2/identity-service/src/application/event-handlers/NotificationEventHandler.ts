...
export class NotificationEventHandler {
   ...
   */
  async handleNotificationDeliveryFailed(event: NotificationDeliveryFailedEvent): Promise<void> {
    try {
      // Idempotency check
      ...
      const consecutiveFailures = await this.getConsecutiveFailureCount(event.userId);
      ...
      if (consecutiveFailures >= 5) {
        this.logger.warn('Locking account due to consecutive notification failures', {
          userId: event.userId,
          consecutiveFailures
        });
      ...
      this.logger.error('Failed to process notification.delivery_failed event', {
        eventId: event.eventId,
        userId: event.userId,
  ...
  private async getConsecutiveFailureCount(userId: string): Promise<number> {
    try {
      ...
      if (error) {
        this.logger.error('Failed to query consecutive failures', { error: error.message });
        return 0;
      }
