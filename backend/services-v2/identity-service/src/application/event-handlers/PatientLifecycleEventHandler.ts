...
import { DeactivateUserUseCase } from '../use-cases/DeactivateUserUseCase';
    ...
    try {
      ...
      await this.deactivateUserUseCase.execute({
        ...
        reason: `Patient deceased on ${event.dateOfDeath.toISOString().split('T')[0]}${
          event.deathCertificateNumber ? ` (Certificate: ${event.deathCertificateNumber})` : ''
        }`,
        terminateSessions: true
      });

      this.logger.info('User account permanently deactivated for deceased patient', {
        userId: event.userId,
        patientId: event.patientId,
        dateOfDeath: event.dateOfDeath,
