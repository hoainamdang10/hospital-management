...
exports.SupabaseRecoveryMethodRepository = void 0;
 ...
 */
class SupabaseRecoveryMethodRepository {
    constructor(supabaseClient, logger) {
        this.supabaseClient = supabaseClient;
        this.logger = logger;
        this.TABLE_NAME = 'recovery_methods';
     ...
     */
    async isRecoveryEmailUsed(email, excludeUserId) {
        try {
            ...
            const { data, error } = await this.supabaseClient.rpc('is_recovery_email_used', {
                p_email: email,
     ...
     */
    async findUserIdByRecoveryEmail(email) {
        try {
            this.logger.info('Finding user by recovery email', { email });
            // Use the helper function from migration
            const { data, error } = await this.supabaseClient.rpc('find_user_by_recovery_email', {
                p_email: email
            });
            if (error) {
     ...
     */
    mapToDomain(row) {
        return RecoveryMethod_1.RecoveryMethod.create({
            userId: row.user_id,
...
exports.SupabaseRecoveryMethodRepository = SupabaseRecoveryMethodRepository;
//# sourceMappingURL=SupabaseRecoveryMethodRepository.js.map