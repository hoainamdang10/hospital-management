...
exports.SupabaseRecoveryHistoryRepository = void 0;
 ...
 */
class SupabaseRecoveryHistoryRepository {
    constructor(supabaseClient, logger) {
        this.supabaseClient = supabaseClient;
        this.logger = logger;
        this.TABLE_NAME = 'recovery_history';
    ...
    async getHistory(filter) {
        try {
            ...
            const attempts = (data || []).map((row) => this.mapToDomain(row));
     ...
     */
    async countRecentAttempts(userId, attemptType, sinceDate) {
        try {
            ...
            const { data, error } = await this.supabaseClient.rpc('count_recent_recovery_attempts', {
                p_user_id: userId,
     ...
     */
    async getRecentFailedAttempts(userId, sinceDate) {
        try {
     ...
     */
    async deleteOldHistory(olderThan) {
        try {
            ...
            // Use the helper function from migration
            const { data, error } = await this.supabaseClient.rpc('cleanup_old_recovery_history', {
     ...
     */
    mapToDomain(row) {
        return RecoveryAttempt_1.RecoveryAttempt.create({
            id: row.id,
            userId: row.user_id,
...
exports.SupabaseRecoveryHistoryRepository = SupabaseRecoveryHistoryRepository;
//# sourceMappingURL=SupabaseRecoveryHistoryRepository.js.map