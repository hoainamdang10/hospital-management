    ...
    async execute(request) {
        try {
            ...
            if (existingUser) {
                ...
                return {
                    success: false,
                    error: "Tài khoản đã được kích hoạt trước đó. Vui lòng đăng nhập hoặc dùng chức năng quên mật khẩu.",
                    errorCode: "USER_ALREADY_EXISTS",
                };
            }
            // 4. Create auth user + profile
            const invitationData = (invitation.invitationData || {});
            const invitationFullName = typeof invitationData.fullName === "string"
                ? invitationData.fullName
                : undefined;
            const invitationPhone = typeof invitationData.phoneNumber === "string"
                ? invitationData.phoneNumber
                : undefined;
            const fullName = request.fullName?.trim().length && request.fullName.trim().length >= 2
                ? request.fullName.trim()
                : invitationFullName || invitation.email.split("@")[0] || "Staff";
            const phoneNumber = request.phoneNumber && request.phoneNumber.trim().length > 0
            ...
            const professionalData = invitation.invitationData;
