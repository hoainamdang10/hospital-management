...
describe('Health Check Integration Tests', () => {
  ...
  describe('Comprehensive Health Check', () => {
    it('should return healthy status when all components are operational', async () => {
      // Mock successful Supabase responses for all tables
      ...
      // Make schema() return the client itself for chaining
      mockSupabaseClient.schema = jest.fn().mockReturnValue(mockSupabaseClient);
  ...
  describe('Circuit Breaker Integration', () => {
    it('should report circuit breaker status correctly', async () => {
      ...
      CircuitBreakerFactory.getBreaker('service-2'); // Create but don't use
  ...
  describe('Error Handling', () => {
    it('should handle network timeouts gracefully', async () => {
      ...
      expect(result.overall).toBe(HealthStatus.UNHEALTHY);
      // Note: logger.error is called inside try-catch, not in the main checkHealth catch
      // So we don't expect it to be called for individual component failures
    });
