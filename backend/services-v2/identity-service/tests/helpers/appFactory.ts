...
export async function createTestApp(config?: TestAppConfig): Promise<{
  ...
  const supabaseJwtSecret =
    config?.supabaseJwtSecret || process.env.SUPABASE_JWT_SECRET!;
  ...
  const mfaService = new SupabaseMFAService(supabaseClient as any, logger);
  ...
  const refreshTokenUseCase = new RefreshTokenUseCase(authService, logger);
  ...
  const checkRoleUseCase = new CheckRoleUseCase(permissionService, logger);

  const checkRolesUseCase = new CheckRolesUseCase(permissionService, logger);
