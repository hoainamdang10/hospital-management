...
export class SupabasePreferencesRepository {
   ...
   */
  public async findByUserId(userId: string): Promise<NotificationPreferences | null> {
    try {
   ...
   */
  public async getOrCreate(userId: string, recipientType: string, email?: string, phoneNumber?: string): Promise<NotificationPreferences> {
   ...
   */
  public async checkRateLimit(userId: string, channel: string): Promise<boolean> {
    const { data, error } = await this.supabase
   ...
   */
  public async updateChannelPreferences(userId: string, enabledChannels: string[]): Promise<void> {
    const { error } = await this.supabase
      .from('notification_preferences')
      .update({
   ...
   * Update quiet hours
   */
  public async updateQuietHours(userId: string, enabled: boolean, start?: string, end?: string): Promise<void> {
    const { error } = await this.supabase
      .from('notification_preferences')
      .update({
   ...
   */
  public async findAll(limit: number = 100, offset: number = 0): Promise<NotificationPreferences[]> {
    const { data, error } = await this.supabase
      .from('notification_preferences')
      .select('*')
      .eq('is_active', true)
