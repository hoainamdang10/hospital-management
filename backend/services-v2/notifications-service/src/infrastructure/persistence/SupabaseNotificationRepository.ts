...
export class SupabaseNotificationRepository implements INotificationRepository {
  ...
  public async update(notification: Notification): Promise<void> {
    try {
      const record = this.mapToRecord(notification);
      const { id, notification_id, created_at, created_by, ...updateData } =
        record;
  ...
  public async findById(id: NotificationId): Promise<Notification | null> {
    try {
      ...
      throw new Error(
        `Failed to find notification by ID: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  }
  ...
  public async findByRecipient(
    ...
    try {
      ...
      throw new Error(
        `Failed to find notifications by recipient: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  }
  ...
  public async findByCriteria(
    ...
    try {
      ...
      if (criteria.scheduledAfter)
        query = query.gte(
          "scheduled_at",
          criteria.scheduledAfter.toISOString(),
        );
      if (criteria.scheduledBefore)
        query = query.lte(
          "scheduled_at",
          criteria.scheduledBefore.toISOString(),
        );
      ...
      throw new Error(
        `Failed to find notifications by criteria: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  }
   ...
   */
  public async findScheduledForProcessing(
    _beforeTime?: Date,
    limit: number = 100,
  ): Promise<Notification[]> {
    try {
      const { data, error } = await this.supabase.rpc(
        "get_scheduled_notifications_due",
        { p_limit: limit },
      );
      ...
      const notificationIds = (data || []).map(
        (row: any) => row.notification_id,
      );
      ...
      throw new Error(
        `Failed to find scheduled notifications: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  }
  ...
  public async findFailedForRetry(
    ...
    try {
      ...
      throw new Error(
        `Failed to find failed notifications for retry: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  }
   ...
   */
  public async findUrgentNotifications(
    limit: number = 100,
  ): Promise<Notification[]> {
    const { data, error } = await this.supabase
      .from("notifications")
  ...
  public async findByHealthcareContext(
    ...
    try {
      ...
      throw new Error(
        `Failed to find by healthcare context: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  }

  public async findByTemplateType(
    templateType: string,
    limit: number = 20,
    offset: number = 0,
  ): Promise<Notification[]> {
  ...
  public async getStatistics(dateRange?: {
    start: Date;
    end: Date;
    ...
    try {
      ...
      return {
        ...
        failureReasons: {},
        recentActivity: {
          last24Hours: notifications.filter(
            (n) =>
              new Date(n.created_at) >
              new Date(Date.now() - 24 * 60 * 60 * 1000),
          ).length,
          lastWeek: notifications.filter(
            (n) =>
              new Date(n.created_at) >
              new Date(Date.now() - 7 * 24 * 60 * 60 * 1000),
          ).length,
          lastMonth: notifications.filter(
            (n) =>
              new Date(n.created_at) >
  ...
  public async getRecipientHistory(
    recipientId: string,
    limit: number = 50,
    offset: number = 0,
  ): Promise<Notification[]> {
  ...
  public async findOverdueNotifications(
    overdueThreshold: number = 60,
  ): Promise<Notification[]> {
  ...
  public async findHighRetryNotifications(
  ...
  public async findByPriorityAndStatus(
  ...
  public async getAverageProcessingTime(
  ...
  public async getFailureRateByChannel(dateRange?: {
    start: Date;
    end: Date;
  }): Promise<Record<string, number>> {
    const { data, error } = await this.supabase.rpc(
      "get_delivery_stats_by_channel",
      {
  ...
  public async getSuccessRateByTemplateType(dateRange?: {
    start: Date;
    end: Date;
  }): Promise<Record<string, number>> {
    let query = this.supabase
  ...
  public async getNotificationTrends(
    ...
    Array<{ date: Date; total: number; successful: number; failed: number }>
  > {
  ...
  private mapToAggregate(record: any): Notification {
    ...
    const persistedNotificationId = NotificationId.fromString(
    ...
    (notification as any).props.notificationId = persistedNotificationId;
    (notification as any)._id = persistedNotificationId.value;
  ...
  private groupByPriority(
    ...
    notifications.forEach((n) => {
      result[n.priority as NotificationPriority] =
  ...
  private calculateAvgDeliveryTime(notifications: any[]): number {
    const withDeliveryTime = notifications.filter(
    ...
    const sum = withDeliveryTime.reduce((acc, n) => {
      const deliveryTime =
        new Date(n.sent_at).getTime() - new Date(n.created_at).getTime();
