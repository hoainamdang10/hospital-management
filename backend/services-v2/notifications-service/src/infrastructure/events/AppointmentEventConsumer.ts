    ...
    try {
      ...
      await this.sendNotificationUseCase.execute({
        ...
        data: {
          ...
          statusMessage:
            "Yêu cầu đặt lịch đã được nhận. Vui lòng thanh toán để xác nhận lịch hẹn.",
        },
      ...
      console.log(
        "[AppointmentEventConsumer] Appointment scheduled notification sent (payment pending)",
        {
          appointmentId: data.appointmentId,
  ...
  private async handleAppointmentConfirmed(
    ...
    try {
      ...
      try {
        ...
        if (!validation.valid) {
          console.error(
            "[AppointmentEventConsumer] Invalid reminders request",
            {
              appointmentId: data.appointmentId,
        ...
        if (result.isSuccess) {
          ...
          console.log(
            "[AppointmentEventConsumer] Reminders created successfully",
            {
              appointmentId: data.appointmentId,
          ...
          console.error(
            "[AppointmentEventConsumer] Failed to create reminders",
            {
              appointmentId: data.appointmentId,
        ...
        console.error(
          "[AppointmentEventConsumer] Failed to create reminders (non-critical)",
          {
            appointmentId: data.appointmentId,
            error:
      ...
      console.log(
        "[AppointmentEventConsumer] Appointment confirmed flow completed",
        {
          appointmentId: data.appointmentId,
      ...
      console.error(
        "[AppointmentEventConsumer] Failed to process appointment confirmed",
        {
          appointmentId: data.appointmentId,
          error: error instanceof Error ? error.message : "Unknown error",
        }
    ...
    try {
      ...
      try {
        ...
        if (cancelResult.isSuccess) {
          ...
          console.error(
            "[AppointmentEventConsumer] Failed to cancel reminders (non-critical)",
            {
        ...
        console.error(
          "[AppointmentEventConsumer] Error cancelling reminders (non-critical)",
          {
            error:
              reminderError instanceof Error
                ? reminderError.message
      ...
      await this.sendNotificationUseCase.execute({
        ...
        data: {
          ...
          statusMessage:
            data.cancellationReason.includes("Payment timeout") ||
      ...
      console.log(
        "[AppointmentEventConsumer] Patient cancellation notification sent",
        {
          appointmentId: data.appointmentId,
          patientId: data.patientId,
        }
      ...
      console.log(
        "[AppointmentEventConsumer] Doctor cancellation notification sent",
        {
          appointmentId: data.appointmentId,
          doctorId: data.doctorId,
        }
      ...
      console.log(
        "[AppointmentEventConsumer] Appointment cancelled flow completed",
        {
          appointmentId: data.appointmentId,
          notificationsSent: 2, // patient + doctor
      ...
      console.error(
        "[AppointmentEventConsumer] Failed to process appointment cancelled",
        {
          appointmentId: data.appointmentId,
          error: error instanceof Error ? error.message : "Unknown error",
        }
  ...
  private async scheduleAppointmentReminders(
    ...
    try {
      ...
      if (result.isSuccess) {
        const { created } = result.getValue()!;
        console.log(
          `[AppointmentEventConsumer] Created ${created} reminder(s) for appointment ${data.appointmentId}`
        );
      } else {
        console.error(
          `[AppointmentEventConsumer] Failed to create reminders: ${result.getError()}`
        );
      }
    } catch (error) {
  ...
  private async sendPrescriptionNotification(
    ...
    try {
      const notificationData = {
        ...
        content:
          "Đơn thuốc từ buổi khám của bạn đã sẵn sàng. Vui lòng đến nhà thuốc để lấy thuốc.",
        channels: this.getEnabledChannels(preferences, ["email", "in_app"]),
        priority: "normal",
        scheduledAt: new Date(),
        metadata: {
  ...
  private async sendLabTestNotification(
    ...
    try {
      const notificationData = {
        ...
        content:
          "Bác sĩ đã chỉ định xét nghiệm. Vui lòng đến phòng xét nghiệm để thực hiện.",
        channels: this.getEnabledChannels(preferences, ["email", "in_app"]),
        priority: "normal",
        scheduledAt: new Date(),
        metadata: {
  ...
  private async updateReminderSchedules(
    ...
    try {
      ...
      if (cancelResult.isSuccess) {
        const cancelledCount = cancelResult.getValue()!;
        console.log(
          `[AppointmentEventConsumer] Cancelled ${cancelledCount} old reminder(s) for rescheduled appointment ${data.appointmentId}`
        );
      } else {
        console.error(
          `[AppointmentEventConsumer] Failed to cancel old reminders: ${cancelResult.getError()}`
        );
      }
      ...
      if (createResult.isSuccess) {
        ...
        console.log(
          `[AppointmentEventConsumer] Created ${created} new reminder(s) for rescheduled appointment ${data.appointmentId}`
        );
      } else {
        console.error(
          `[AppointmentEventConsumer] Failed to create new reminders: ${createResult.getError()}`
        );
      }
    } catch (error) {
  ...
  private async sendRescheduleOffer(
    ...
    try {
      const notificationData = {
        ...
        content:
          "Chúng tôi xin lỗi vì sự bất tiện. Bạn có thể đặt lịch hẹn mới mà không tốn phí.",
        channels: this.getEnabledChannels(preferences, ["email", "in_app"]),
        priority: "normal",
        scheduledAt: new Date(),
        metadata: {
