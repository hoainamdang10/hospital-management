...
export class VietnameseTemplateService implements ITemplateService {
  ...
  private readonly templateUsage: Map<string, number> = new Map();
...
{{hospitalName}}`,
   ...
   */
  public async applyTemplate(request: TemplateRequest): Promise<TemplateResult> {
    try {
      ...
      const missingPlaceholders = template.placeholders.filter(
        placeholder => !(placeholder in request.templateData)
      );
      ...
      const notificationTemplate = NotificationTemplate.create({
   ...
   */
  private processTemplate(content: any, data: Record<string, any>): any {
   ...
   * Process loop blocks
   */
  private processLoops(text: string, data: Record<string, any>): string {
   ...
   */
  private truncateForPush(content: string): string {
    const maxLength = 100;
    const cleanContent = content.replace(/<[^>]*>/g, '').replace(/\n\s*\n/g, ' ').trim();
   ...
   */
  public async getTemplatesByType(templateType: string): Promise<NotificationTemplate[]> {
    try {
   ...
   */
  public async getTemplateAnalytics(dateRange?: { start: Date; end: Date }): Promise<TemplateAnalytics> {
    try {
   ...
   * Get channel support
   */
  private getChannelSupport(): Record<string, number> {
