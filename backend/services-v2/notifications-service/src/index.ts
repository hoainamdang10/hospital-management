...
async function initializeEventConsumers(container: DIContainer): Promise<void> {
  try {
    ...
    const appointmentEventConsumer = container.resolve(
      ServiceTokens.APPOINTMENT_EVENT_CONSUMER,
    ) as any;
    const staffEventConsumer = container.resolve(
      ServiceTokens.STAFF_EVENT_CONSUMER,
    ) as any;
    const billingEventConsumer = container.resolve(
    ...
    console.log(
      "   - Appointment Event Consumer (scheduled, confirmed, cancelled, rescheduled, completed, reminder.*)",
    );
 ...
 */
async function initializeReminderCronJob(
  container: DIContainer,
): Promise<void> {
  try {
    console.log("⏰ Initializing Reminder Cron Job...");

    const reminderCronJob = container.resolve(
...
async function bootstrap() {
  try {
    ...
    app.get("/health", async (_req: Request, res: Response) => {
      try {
        ...
        const consumersStatus = {
          appointment:
            appointmentEventConsumer.isConsumerConnected?.() || false,
          staff: staffEventConsumer.isConsumerConnected?.() || false,
        };

        const allConsumersConnected = Object.values(consumersStatus).every(
          (status) => status,
        );
    ...
    const server = app.listen(PORT, () => {
      ...
      console.log(
        `🔗 API Base URL: http://localhost:${PORT}/api/v1/notifications`,
      );
    ...
    process.on("SIGTERM", async () => {
      console.log(
        "SIGTERM signal received: closing HTTP server and event consumers",
      );
    ...
    process.on("SIGINT", async () => {
      console.log(
        "SIGINT signal received: closing HTTP server and event consumers",
      );
