...
class ClinicalEMREventConsumer {
        ...
        try {
            ...
            switch (routingKey) {
                case 'clinical.patient.profile.updated':
                    await this.handlePatientClinicalProfileUpdated(event.payload);
                    break;
                case 'clinical.treatment.plan.created':
                    await this.handleTreatmentPlanCreated(event.payload);
                    break;
                case 'clinical.test.ordered':
                    await this.handleMedicalTestOrdered(event.payload);
                    break;
                case 'clinical.test.result.ready':
                    await this.handleMedicalTestResultReady(event.payload);
                    break;
                case 'clinical.document.added':
                    await this.handleClinicalDocumentAdded(event.payload);
                    break;
                case 'clinical.vitals.recorded':
                    await this.handleVitalSignsRecorded(event.payload);
                    break;
                case 'clinical.medication.prescribed':
                    await this.handleMedicationPrescribed(event.payload);
                    break;
                case 'clinical.emergency.alert':
                    await this.handleEmergencyAlertTriggered(event.payload);
                    break;
                default:
     ...
     */
    async handleMedicalTestOrdered(data) {
     ...
     */
    async handleMedicalTestResultReady(data) {
     ...
     */
    async handleClinicalDocumentAdded(data) {
     ...
     */
    async handleVitalSignsRecorded(data) {
     ...
     */
    async handleMedicationPrescribed(data) {
     ...
     */
    async handleEmergencyAlertTriggered(data) {
     ...
     * Send clinical profile update notification to patient
     */
    async sendClinicalProfileUpdateNotification(data, preferences) {
        try {
            const riskLevelText = {
                'low': 'thấp',
                'medium': 'trung bình',
     ...
     */
    async sendHighRiskProfileNotification(data) {
        try {
            if (!data.clinicalData.primaryPhysician)
                return;
            const notificationData = {
     ...
     */
    async sendAllergyAlertNotification(data) {
        try {
            const notificationData = {
                recipientId: data.patientId,
                recipientType: 'patient',
     ...
     * Send treatment plan notification to patient
     */
    async sendTreatmentPlanNotification(data, preferences) {
        try {
            const notificationData = {
                recipientId: data.patientId,
                recipientType: 'patient',
     ...
     */
    async sendTreatmentPlanPhysicianNotification(data, preferences) {
        try {
            const notificationData = {
     ...
     * Schedule treatment reminders
     */
    async scheduleTreatmentReminders(data, preferences) {
        try {
     ...
     * Send test order notification to patient
     */
    async sendTestOrderNotification(data, preferences) {
        try {
            const urgencyText = {
                'routine': 'thường quy',
                'urgent': 'khẩn',
     ...
     * Send urgent test notification to physician
     */
    async sendUrgentTestNotification(data, preferences) {
        try {
            const notificationData = {
                recipientId: data.physicianId,
                recipientType: 'staff',
     ...
     */
    async sendTestPreparationInstructions(data, preferences) {
        try {
     ...
     * Send test result notification to patient
     */
    async sendTestResultNotification(data, preferences) {
        try {
            const statusText = {
                'normal': 'bình thường',
                'abnormal': 'bất thường',
     ...
     * Send critical result notification to physician
     */
    async sendCriticalResultNotification(data, preferences) {
        try {
            let criticalInfo = '';
     ...
     */
    async sendTestFollowUpNotification(data, preferences) {
        try {
            const notificationData = {
                recipientId: data.patientId,
                recipientType: 'patient',
     ...
     * Send document notification to patient
     */
    async sendDocumentNotification(data, preferences) {
        try {
     ...
     */
    async sendDocumentPhysicianNotification(data, preferences) {
        try {
            if (!data.physicianId) {
     ...
     */
    async sendDocumentFollowUpNotification(data, preferences) {
        try {
            const notificationData = {
                recipientId: data.patientId,
                recipientType: 'patient',
     ...
     */
    async handleVitalSignsAlerts(data) {
        try {
            if (!data.alerts || data.alerts.length === 0)
                return;
     ...
     */
    async sendVitalSignsNotification(data, preferences) {
        try {
     ...
     */
    async sendCriticalVitalSignsAlert(data, alert) {
        try {
            const notificationData = {
     ...
     */
    async sendVitalSignsAlertNotification(data, alert) {
        try {
            const notificationData = {
                recipientId: data.patientId,
                recipientType: 'patient',
    ...
    async sendPrescriptionNotification(data, preferences) {
        try {
            const medicationList = data.medications.map(med => `${med.name} (${med.dosage}, ${med.frequency})`).join(', ');
            const notificationData = {
                recipientId: data.patientId,
                recipientType: 'patient',
     ...
     */
    async sendPharmacyPickupNotification(data, preferences) {
        try {
            const notificationData = {
                recipientId: data.patientId,
                recipientType: 'patient',
     ...
     */
    async scheduleMedicationReminders(data, preferences) {
        try {
     ...
     */
    async sendEmergencyAlertNotification(data) {
        try {
            const alertTypeText = {
     ...
     */
    async sendEmergencyContactNotification(data) {
        try {
            const notificationData = {
                recipientId: data.patientId,
                recipientType: 'patient',
     ...
     * Send department emergency notification
     */
    async sendDepartmentEmergencyNotification(data) {
        try {
            const notificationData = {
     ...
     * Log emergency notification for compliance
     */
    async logEmergencyNotification(data) {
        try {
            const notificationData = {
    ...
    getTreatmentFrequency(frequency) {
        const frequencyMap = {
            'daily': 1,
            'weekly': 7,
            'biweekly': 14,
    ...
    getNextTreatmentDate(currentDate, frequencyDays) {
