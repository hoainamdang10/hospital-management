    ...
    async execute(request) {
        try {
            ...
            if (!request.appointmentId || !request.patientId) {
                return Result_1.Result.fail('Appointment ID and Patient ID are required');
            }
            if (!request.appointmentDate || !request.appointmentTime) {
                return Result_1.Result.fail('Appointment date and time are required');
            }
            if (!request.patientPhone && !request.patientEmail) {
                return Result_1.Result.fail('Patient must have at least one contact method');
            }
            ...
            if (appointmentDateTime <= new Date()) {
                ...
                return Result_1.Result.ok({ created: 0 });
            }
            ...
            for (const reminderType of reminderTypes) {
                ...
                if (scheduledSendTime > new Date()) {
                    const reminderResult = AppointmentReminder_1.AppointmentReminder.create({
                    ...
                    if (reminderResult.isFailure) {
                        console.error(`[CreateAppointmentRemindersUseCase] Failed to create reminder: ${reminderResult.getError()}`);
                        continue;
                    }
                    const reminder = reminderResult.getValue();
                    const saveResult = await this.reminderRepo.save(reminder);
                    if (saveResult.isFailure) {
                        console.error(`[CreateAppointmentRemindersUseCase] Failed to save reminder: ${saveResult.getError()}`);
                        continue;
                    }
                    created++;
            ...
            return Result_1.Result.ok({ created });
        }
        catch (error) {
            console.error('[CreateAppointmentRemindersUseCase] Unexpected error:', error);
            return Result_1.Result.fail(`Failed to create reminders: ${error.message}`);
        }
    }
    /**
