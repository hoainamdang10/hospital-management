...
class CreateTemplateUseCase {
    ...
    async execute(request) {
        ...
        try {
            const template = NotificationTemplate_1.NotificationTemplate.create({
                templateId: crypto.randomUUID(),
                templateType: request.type, // Cast to TemplateType
                name: request.name,
                description: `${request.name} template`,
                language: request.language,
                priority: 'NORMAL', // Cast to TemplatePriority
                content: {
