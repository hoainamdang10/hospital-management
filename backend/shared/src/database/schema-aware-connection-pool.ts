 ...
 */
export class SchemaAwareConnectionPool {
  private connections: Map<string, SchemaConnection[]> = new Map();
  ...
  async initialize(): Promise<void> {
    ...
    logger.info("🔄 Initializing Schema-Aware Connection Pool...");
    ...
    try {
      ...
      logger.info("✅ Schema-Aware Connection Pool initialized successfully");
    } catch (error) {
  ...
  async getConnection(
    serviceName: string
  ): Promise<SupabaseClient<any, string, any>> {
    if (!this.isInitialized) {
      await this.initialize();
    }

    const schemaName = getSchemaForService(serviceName);
    ...
    let connection = connections.find(
      (conn) => Date.now() - conn.lastUsed.getTime() > 1000 // 1 second idle
    );
    ...
    if (!connection) {
      ...
      if (connections.length < this.config.maxConnections) {
        ...
        connection = connections.reduce((oldest, current) =>
  ...
  private async createConnection(
    ...
    try {
      ...
      logger.debug(
        `✅ Created new connection for ${serviceName} -> ${schemaName}`
      );

      return connection;
    } catch (error) {
      logger.error(`❌ Failed to create connection for ${serviceName}:`, error);
      throw error;
    }
  }
   ...
   */
  private async createSchemaConnections(
    schemaName: string,
    count: number
  ): Promise<void> {
        ...
        if (prop === "from") {
          return (tableName: string) => {
            ...
            if (!validateTableAccess(serviceName, tableName)) {
              ...
              logger.error("🚨 Schema Access Violation:", {
                serviceName,
                tableName,
            ...
            logger.debug("✅ Schema access granted:", {
              serviceName,
              tableName,
              schemaName: connection.schemaName,
            });
  ...
  async executeFHIRValidation<T>(
    serviceName: string,
    queryFn: (client: SupabaseClient<any, string, any>) => Promise<T>
  ): Promise<T> {
    ...
    try {
      ...
      logger.info("🏥 FHIR-compliant query executed:", {
        serviceName,
        responseTime,
        timestamp: new Date().toISOString(),
      });
      ...
      logger.error("❌ FHIR query failed:", {
        serviceName,
   ...
   */
  private getServiceNameForSchema(schemaName: string): string {
    const mapping = {
    ...
    return mapping[schemaName as keyof typeof mapping] || "unknown-service";
  }

  /**
   * Update connection metrics
   */
  private updateMetrics(responseTime: number, hasError: boolean): void {
  ...
  getMetrics(): ConnectionMetrics {
    ...
    for (const connections of this.connections.values()) {
      totalIdle += connections.filter(
        (conn) =>
          Date.now() - conn.lastUsed.getTime() > this.config.idleTimeoutMillis
      ).length;
    }
  ...
  async healthCheck(): Promise<{
    ...
    try {
      ...
      const status =
        metrics.errorCount / Math.max(metrics.totalQueries, 1) < 0.1
          ? "healthy"
          : "degraded";
  ...
  async shutdown(): Promise<void> {
    logger.info("🔄 Shutting down Schema-Aware Connection Pool...");
...
// Singleton instance
let poolInstance: SchemaAwareConnectionPool | null = null;

/**
 * Get or create the connection pool instance
 */
export function getConnectionPool(): SchemaAwareConnectionPool {
  if (!poolInstance) {
