...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      logger.info("Searching ICD-10 codes:", { query, limit });

      const results = await healthcareService.searchICD10Codes(
        query as string,
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      logger.info("Getting ICD-10 codes by category:", category);
...
router.get("/categories", async (req: Request, res: Response) => {
  try {
    logger.info("Getting all ICD-10 categories");
    ...
    logger.error("Error getting ICD-10 categories:", error);
    res.status(500).json({
      success: false,
      error: {
...
router.get("/stats", async (req: Request, res: Response) => {
  try {
    logger.info("Getting ICD-10 usage statistics");
    ...
    logger.error("Error getting ICD-10 statistics:", error);
    res.status(500).json({
      success: false,
      error: {
