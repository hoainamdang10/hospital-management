...
export default function ChatBot() {
    ...
    const [messages, setMessages] = useState<Message[]>([
        {
            role: 'assistant',
            content: 'Xin chào! Tôi là AI Assistant của bệnh viện. Tôi có thể giúp bạn đặt lịch khám, tìm bác sĩ, hoặc xem lịch hẹn. Bạn cần hỗ trợ gì?',
            timestamp: new Date(),
        },
    ]);
            ...
            <AnimatePresence>
                {!isOpen && (
                    <motion.button
                        ...
                        className="fixed bottom-6 right-6 z-50 flex h-16 w-16 items-center justify-center rounded-full bg-gradient-to-br from-blue-500 to-purple-600 text-white shadow-2xl transition-all hover:shadow-blue-500/50"
                    >
            ...
            <AnimatePresence>
                {isOpen && (
                    <motion.div
                        ...
                        className="fixed bottom-6 right-6 z-50 flex w-[400px] flex-col overflow-hidden rounded-2xl bg-white shadow-2xl dark:bg-gray-900"
                    ...
                    >
                        ...
                        {!isMinimized && (
                            <>
                                <div className="flex-1 space-y-4 overflow-y-auto p-4 bg-gray-50 dark:bg-gray-800/50">
                                ...
                                <div className="border-t border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-900">
                                    <div className="flex items-end gap-2">
                                        <textarea
                                            ...
                                            rows={1}
                                            disabled={isLoading}
                                            className="flex-1 resize-none rounded-xl border border-gray-300 bg-gray-50 px-4 py-2 text-sm focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/20 disabled:opacity-50 dark:border-gray-600 dark:bg-gray-800 dark:text-white"
                                            style={{ maxHeight: '120px' }}
                                        />
                                        <button
                                            onClick={handleSend}
                                            disabled={!input.trim() || isLoading}
                                            className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-xl bg-gradient-to-r from-blue-500 to-purple-600 text-white transition-all hover:shadow-lg hover:shadow-blue-500/50 disabled:opacity-50 disabled:cursor-not-allowed"
                                        >
                                            {isLoading ? (
                                    ...
                                    <p className="mt-2 text-xs text-gray-500 dark:text-gray-400">
                                        Nhấn <kbd className="rounded bg-gray-200 px-1 dark:bg-gray-700">Enter</kbd> để gửi
                                    </p>
                                </div>
                            </>
                        )}
