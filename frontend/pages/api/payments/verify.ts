...
export default async function handler(req: NextApiRequest, res: NextApiResponse<PaymentVerifyResponse>) {
  ...
  try {
    ...
    const response = await fetch(`${apiGatewayUrl}/api/payments/verify?orderCode=${encodeURIComponent(orderCode)}`, {
      method: 'GET',
      headers: {
    ...
    if (!response.ok) {
      console.error('Payment verify error:', responseData);
